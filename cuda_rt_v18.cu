// ═══════════════════════════════════════════════════════════════════════════
// V18 — WARP-PERSISTENT RAY RECYCLING (Diffuse Efficiency Target: ≥70%)
//
// Problem: Diffuse warp efficiency = 42.7% (13.7/32 threads active)
// Cause: Rays terminate at different depths → idle lanes waste SIMD
//
// Solution: Per-lane ray recycling WITHOUT extra state (v15's fatal flaw)
//   - Each lane runs a FULL ray to completion (DFS, same as v12)
//   - When done, lane writes result and IMMEDIATELY grabs next ray
//   - Uses per-lane atomicAdd (not per-warp) for max fill
//   - Key difference from v15: no per-iteration ballot/refill overhead
//     Instead: each lane independently loops: trace→write→fetch→trace
//   - The warp stays alive as long as ANY lane has work
//   - Trick: use __ballot_sync ONLY at the outer loop to check warp done
//
// Register budget: Must stay at 48 regs / 0 spills
// The secret: ray state (ox,oy,oz,dx,dy,dz,ix,iy,iz,hitT,hitTri,hitU,hitV)
// fits in 13 regs. Stack[8] = 8 regs. sp,ni = 2 regs. Total ray state = 23.
// BVH temps reuse the same regs each iteration. Feasible!
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══ KERNEL A: BASELINE (v12 Incoherent, warp-batch 32) ═══
...
// ═══ KERNEL B: WARP-PERSISTENT RAY RECYCLING ═══
// Each lane runs full DFS trace, then IMMEDIATELY gets a new ray.
// No per-node ballot — just per-ray-completion ballot.
// The warp stays alive until global queue is empty AND all lanes done.
__global__ void __launch_bounds__(256,5) traceRecycle(
    const int4*__restrict__ d_bvh4,int n4,int _u,
    const float*__restrict__ tv0x,const float*__restrict__ tv0y,const float*__restrict__ tv0z,
    const float*__restrict__ tv1x,const float*__restrict__ tv1y,const float*__restrict__ tv1z,
    const float*__restrict__ tv2x,const float*__restrict__ tv2y,const float*__restrict__ tv2z,
    ...
    Hit*__restrict__ hits,int numRays,unsigned long long*__restrict__ stats)
{
    ...
    // Each lane independently fetches and traces rays in a tight loop
    while(true) {
        // ── FETCH: Each lane gets its own ray via per-lane atomic ──
        // Use warp-cooperative fetch: one atomic per warp, distribute via shuffle
        int bs;
        if(lane==0) bs=atomicAdd(&g_rayCounterB, 32);
        bs=__shfl_sync(0xFFFFFFFF, bs, 0);
        int ri = bs + lane;
        if(bs >= numRays) break;  // entire warp out of rays
        ...
        if(ri < numRays) {
            ...
            // ── FULL DFS TRACE (identical to baseline) ──
            while(true){
                if(ni==-1){if(sp>0)ni=stk[--sp];else break;continue;}
                if(ni<=-2){int val=-(ni+2),tc=(val&7)+1,ts=val>>3;
                    for(int i=0;i<tc;i++){int ti=ts+i;float v0x=tv0x[ti],v0y=tv0y[ti],v0z=tv0z[ti];float e1x=tv1x[ti]-v0x,e1y=tv1y[ti]-v0y,e1z=tv1z[ti]-v0z;float e2x=tv2x[ti]-v0x,e2y=tv2y[ti]-v0y,e2z=tv2z[ti]-v0z;float hx=dy*e2z-dz*e2y,hy=dz*e2x-dx*e2z,hz=dx*e2y-dy*e2x;float a2=e1x*hx+e1y*hy+e1z*hz;if(fabsf(a2)<1e-8f)continue;float f=__frcp_rn(a2);float sx=ox-v0x,sy=oy-v0y,sz=oz-v0z;float u=f*(sx*hx+sy*hy+sz*hz);if(u<0.f||u>1.f)continue;float qx=sy*e1z-sz*e1y,qy=sz*e1x-sx*e1z,qz=sx*e1y-sy*e1x;float v=f*(dx*qx+dy*qy+dz*qz);if(v<0.f||u+v>1.f)continue;float tt=f*(e2x*qx+e2y*qy+e2z*qz);if(tt>0.001f&&tt<hitT){hitT=tt;hitTri=ti;hitU=u;hitV=v;}}
        ...
        // Lane immediately loops back to fetch next batch of 32
    }
}
...
int main(){
    printf("══════════════════════════════════════════════════════════════════════════\n");
    printf("  V18 — Warp-Persistent Ray Recycling\n");
    printf("  Each lane independently: trace → write → fetch next → trace\n");
    printf("  Target: diffuse warp efficiency 42.7%% → 70%%+\n");
    printf("══════════════════════════════════════════════════════════════════════════\n\n");
    ...
    for(int sceneK:{100,500}){
        ...
        BVH4H*h_b4=(BVH4H*)calloc(nc,sizeof(BVH4H));int n4=0;collapse4(h_ord,0,h_b4,&n4);
        ...
        for(int test=0;test<3;test++){
            ...
            BR recy=benchK(traceRecycle,true,rays,numRays,d_bvh4,n4,d_v,prop);
            double delta=(recy.mr-base.mr)/base.mr*100;
            printf("  %-20s Base: %6.0f  Recy: %6.0f  %+.1f%%  (hit: %d%%/%d%%)\n",
                name,base.mr,recy.mr,delta,base.hitPct,recy.hitPct);
        }
        printf("\n");
