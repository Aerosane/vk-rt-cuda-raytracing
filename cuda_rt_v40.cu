/* v40 — BVH4 Unified — SAH + Leaf Tuning
 *
 * Changes from v39:
 *   - SAH bins: 16 → 32 (tighter BVH, fewer node visits)
 *   - Leaf size sweep: try max_leaf=3,4,6,8
 *   - Added node visit counter for analysis
 *
 * Target: 1200+ MR/s diffuse @ 100K
 */
...
// BVH4 diffuse — 128 threads/block for higher occupancy (8+ blocks/SM vs 5)
__global__ void __launch_bounds__(128, 8) traceBVH4Diffuse128(
    const int4* __restrict__ bvh, int n4,
    const float* __restrict__ tv0x, const float* __restrict__ tv0y, const float* __restrict__ tv0z,
    const float* __restrict__ tv1x, const float* __restrict__ tv1y, const float* __restrict__ tv1z,
    const float* __restrict__ tv2x, const float* __restrict__ tv2y, const float* __restrict__ tv2z,
...
// BVH4 diffuse with Woop triangles — DEAD END: 55-70% slower than MT (SoA wins on SIMT cache coalescing)
// Kept for reference only
#if 0
__global__ void __launch_bounds__(256, 4) traceBVH4DiffuseWoop(
    const int4* __restrict__ bvh, int n4,
    const float4* __restrict__ woopTris,
    const Ray* __restrict__ rays, HitCWBVH* __restrict__ hits, int rayCount, int* __restrict__ finishedCount)
{
    const unsigned lane = threadIdx.x & 31;
    while (true) {
        ...
        while(sp>0&&alive){
            ...
            if(ni<0){
                ...
                for(int t=0;t<tc;t++){
                    int ti=ts+t;
                    int wa = ti * 3;  // Woop address (3 float4s per tri)
                    float4 v00 = __ldg(woopTris + wa + 0);
                    float4 v11 = __ldg(woopTris + wa + 1);
                    float4 v22 = __ldg(woopTris + wa + 2);
                    // Woop intersection (same as CWBVH kernel)
                    float Oz = v00.w - ox*v00.x - oy*v00.y - oz*v00.z;
                    float invDz = 1.0f / (dx*v00.x + dy*v00.y + dz*v00.z);
                    float tt = Oz * invDz;
                    float Ox = v11.w + ox*v11.x + oy*v11.y + oz*v11.z;
                    float Dx = dx*v11.x + dy*v11.y + dz*v11.z;
                    float uu = Ox + tt * Dx;
                    float Oy = v22.w + ox*v22.x + oy*v22.y + oz*v22.z;
                    float Dy = dx*v22.x + dy*v22.y + dz*v22.z;
                    float vv = Oy + tt * Dy;
                    if(tt>0.001f && tt<tHit && uu>=0.f && uu<=1.f && vv>=0.f && uu+vv<=1.f){
                        tHit=tt; hitTri=ti; hitU=uu; hitV=vv;
                    }
                }
                continue;
            }
...
struct BVH2Builder {
    ...
    int maxLeaf = 3;  // configurable max triangles per leaf
    ...
    int buildRec(std::vector<int>&idx,int s,int e){
        ...
        int cnt=e-s;
        if(cnt<=maxLeaf){nd.triStart=(int)ordered.size();nd.triCount=cnt;
        ...
        for(int ax=0;ax<3;ax++){
            ...
            const int NB=32; AABB lBox[NB],rBox[NB]; int lCnt[NB],rCnt[NB];
...
int main() {
    ...
    // Leaf size sweep (only for 500K scene)
    float sceneMin = -5.f, sceneMax = 5.f;
    float sceneInvSize = 1.f / (sceneMax - sceneMin);
    printf("\n  ═══ Leaf Size Sweep (500K tris) ═══\n");
    {
        int NTRI = 500000;
        Tri* h_tris = (Tri*)malloc(NTRI * sizeof(Tri));
        NTRI = genScene(h_tris, NTRI, 10.f);

        int leafSizes[] = {2, 3, 4, 6, 8};
        for (int ls : leafSizes) {
            BVH2Builder bvh2; bvh2.maxLeaf = ls; bvh2.build(h_tris, NTRI);
            int numOrd = (int)bvh2.ordered.size();

            int maxB4N = (int)bvh2.nodes.size();
            BVH4Node* h_bvh4 = (BVH4Node*)calloc(maxB4N, sizeof(BVH4Node));
            int numB4 = 0;
            collapseToB4(bvh2, 0, h_bvh4, numB4, nullptr);
            ...
            cub::DeviceRadixSort::SortPairs(nullptr, cubTempSize, d_mortonKeys, d_mortonKeysOut, d_mortonIdx, d_mortonIdxOut, NRAYS);
            void* d_cubTemp; CK(cudaMalloc(&d_cubTemp, cubTempSize));
            cub::DeviceRadixSort::SortPairs(d_cubTemp, cubTempSize, d_mortonKeys, d_mortonKeysOut, d_mortonIdx, d_mortonIdxOut, NRAYS);
            reorderRays<<<(NRAYS+255)/256,256>>>(d_rays,d_sortedRays,d_mortonIdxOut,NRAYS);
            CK(cudaDeviceSynchronize());

            // Warmup + bench
            resetFinished<<<1,1>>>(d_finishedCount);
            traceBVH4Diffuse<<<640,256>>>(d_bvh4,(int)(numB4*4),
                d_tv0x,d_tv0y,d_tv0z,d_tv1x,d_tv1y,d_tv1z,d_tv2x,d_tv2y,d_tv2z,
                d_sortedRays,d_hits,NRAYS,d_finishedCount);
            CK(cudaDeviceSynchronize());
            ...
            float MRs = (float)NRAYS*iters/(ms*1000.f);
            printf("    leaf=%d: %d B4 nodes (%.0fKB), %d tris → %6.0f MR/s (%.2fms)\n",
                   ls, numB4, numB4*64/1024.f, numOrd, MRs, ms/iters);
            ...
            CK(cudaFree(d_rays));CK(cudaFree(d_sortedRays));CK(cudaFree(d_hits));
            CK(cudaFree(d_finishedCount));
    ...
    printf("\n  ═══ Main Benchmark (leaf=%d, SAH bins=32) ═══\n", 3);
    for(int si = 0; si < 4; si++) {
        ...
        // Build Woop triangles in BVH4's ordered triangle sequence
        float4 *d_bvh4Woop;
        {
            std::vector<float4> b4woop(numOrd * 3);
            for(int i=0;i<numOrd;i++){
                float3a v0=bvh2.ordered[i].v0, v1=bvh2.ordered[i].v1, v2=bvh2.ordered[i].v2;
                float e1x=v0.x-v2.x,e1y=v0.y-v2.y,e1z=v0.z-v2.z;
                float e2x=v1.x-v2.x,e2y=v1.y-v2.y,e2z=v1.z-v2.z;
                float cx=e1y*e2z-e1z*e2y, cy=e1z*e2x-e1x*e2z, cz=e1x*e2y-e1y*e2x;
                float d=e1x*(e2y*cz-e2z*cy)-e1y*(e2x*cz-e2z*cx)+e1z*(e2x*cy-e2y*cx);
                ...
                float r1x=id*(e2z*cx-e2x*cz),r1y=id*(e1x*cz-e1z*cx),r1z=id*(e1z*e2x-e1x*e2z);
                float r2x=id*(e2x*cy-e2y*cx),r2y=id*(e1y*cx-e1x*cy),r2z=id*(e1x*e2y-e1y*e2x);
                b4woop[i*3+0]=make_float4(r2x,r2y,r2z,+(r2x*v2.x+r2y*v2.y+r2z*v2.z));
                b4woop[i*3+1]=make_float4(r0x,r0y,r0z,-(r0x*v2.x+r0y*v2.y+r0z*v2.z));
                b4woop[i*3+2]=make_float4(r1x,r1y,r1z,-(r1x*v2.x+r1y*v2.y+r1z*v2.z));
            }
            CK(cudaMalloc(&d_bvh4Woop, numOrd*3*sizeof(float4)));
            CK(cudaMemcpy(d_bvh4Woop, b4woop.data(), numOrd*3*sizeof(float4), cudaMemcpyHostToDevice));
        }
        ...
        // BVH4 diffuse block count sweep (256 threads/block)
        int blockCounts[] = {160, 320, 640, 960};
        for(int bc : blockCounts) {
            CK(cudaEventRecord(t0));
            for(int i=0;i<iters;i++){
                resetFinished<<<1,1>>>(d_finishedCount);
                traceBVH4Diffuse<<<bc,256>>>(d_bvh4,(int)(numB4*4),
                    d_tv0x,d_tv0y,d_tv0z,d_tv1x,d_tv1y,d_tv1z,d_tv2x,d_tv2y,d_tv2z,
                    d_sortedRays,d_cwbvhHits,NRAYS,d_finishedCount);
            }
            CK(cudaEventRecord(t1));CK(cudaEventSynchronize(t1));
            float sweepMs; CK(cudaEventElapsedTime(&sweepMs,t0,t1));
            float sweepMRs = (float)NRAYS*iters/(sweepMs*1000.f);
            printf("    BVH4×256 %3d blks: %.2fms → %6.0f MR/s\n", bc, sweepMs/iters, sweepMRs);
        }

        // BVH4 diffuse 128 threads/block sweep
        int blockCounts128[] = {320, 640, 960, 1280};
        for(int bc : blockCounts128) {
            CK(cudaEventRecord(t0));
            for(int i=0;i<iters;i++){
                resetFinished<<<1,1>>>(d_finishedCount);
                traceBVH4Diffuse128<<<bc,128>>>(d_bvh4,(int)(numB4*4),
                    d_tv0x,d_tv0y,d_tv0z,d_tv1x,d_tv1y,d_tv1z,d_tv2x,d_tv2y,d_tv2z,
                    d_sortedRays,d_cwbvhHits,NRAYS,d_finishedCount);
            }
            CK(cudaEventRecord(t1));CK(cudaEventSynchronize(t1));
            float sweepMs; CK(cudaEventElapsedTime(&sweepMs,t0,t1));
            float sweepMRs = (float)NRAYS*iters/(sweepMs*1000.f);
            printf("    BVH4×128 %4d blks: %.2fms → %6.0f MR/s\n", bc, sweepMs/iters, sweepMRs);
        }

        printf("    Ratio BVH4sort/pri: %.1f%%\n", 100.f*b4dMRs/priMRs);

        // Reference numbers
        printf("    ── vs v38 BVH4: diff=%.0f MR/s\n",
               si==0?1073.f:si==1?910.f:si==2?816.f:755.f);
        printf("    ── vs OptiX 8.1: pri=%d diff=%d MR/s\n\n",
               si==0?3642:si==1?3038:si==2?2640:0,
               si==0?551:si==1?336:si==2?292:0);

        CK(cudaFree(d_bvh4));CK(cudaFree(d_bvh4Woop));
        CK(cudaFree(d_tv0x));CK(cudaFree(d_tv0y));CK(cudaFree(d_tv0z));
        CK(cudaFree(d_tv1x));CK(cudaFree(d_tv1y));CK(cudaFree(d_tv1z));
        CK(cudaFree(d_tv2x));CK(cudaFree(d_tv2y));CK(cudaFree(d_tv2z));
