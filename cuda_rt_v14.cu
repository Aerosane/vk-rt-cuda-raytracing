// V14 ENGINE: HBM2-Aware L2 Residency Control
//
// HBM2 STACKING PHYSICS:
//   4 stacks × 8 pseudo-channels = 32 channels, 4096-bit bus
//   TSVs: ~100ns DRAM latency, 900 GB/s peak
//   L2: 6 MB, ~3.1 TB/s internal bandwidth, ~200 cycle latency
//
// KEY INSIGHT: L2 EVICTION CONTROL
//   BVH data (0.89-4.33 MB) → accessed ~20× per ray → MUST stay in L2
//   Ray data (144 MB for 4M rays) → accessed ONCE per ray → should NOT pollute L2
//   Triangle data (~few MB) → accessed ~3× per ray → normal caching
//
// TECHNIQUE: Use PTX cache operators to control L2 residency:
//   ld.global.cs (Cache Streaming) for ray data → bypasses L2, uses L1 streaming path
//   ld.global.ca (__ldg) for BVH data → cached in L1 AND L2 (persistent)
//   prefetch.global.L2 for BVH root → pre-warm critical path
//
// This exploits the HBM2 stacked architecture: by keeping BVH in L2,
// we avoid the ~100ns HBM2 round-trip for the most-accessed data,
// while ray data (accessed once) streams directly from HBM2 through L1.
//
// EXPECTED IMPACT: Most benefit for incoherent rays on large scenes (499K)
// where BVH (4.33 MB) barely fits in L2 and ray data evicts it.
...
// ═══ STREAMING LOAD: bypass L2 for ray data ═══
// On Volta, ld.global.cs uses the streaming path:
// HBM2 → L2 (not cached, just pass through) → L1 → register
// This prevents 144MB of ray data from evicting BVH from L2
__device__ __forceinline__ float ldcs(const float*__restrict__ ptr){
    float v;
    asm("ld.global.cs.f32 %0, [%1];" : "=f"(v) : "l"(ptr));
    return v;
}

// ═══ BVH L2 PREFETCH ═══
// Pre-warm BVH root + first few nodes in L2 before traversal starts
__device__ __forceinline__ void prefetchL2(const void*ptr){
...
// ═══ KERNEL A: BASELINE (normal loads, identical to v12 Incoherent) ═══
...
// ═══ KERNEL B: HBM2-AWARE (streaming ray loads + BVH prefetch) ═══
// Identical traversal logic, but:
// 1. Ray data loaded with ld.global.cs (bypasses L2)
// 2. BVH root prefetched to L2 at ray start
// 3. First child prefetched to L1 after AABB test
__global__ void __launch_bounds__(256,5) traceHBM2(
    const int4*__restrict__ d_bvh4,int n4,int _unused,
    const float*__restrict__ tv0x,const float*__restrict__ tv0y,const float*__restrict__ tv0z,
    const float*__restrict__ tv1x,const float*__restrict__ tv1y,const float*__restrict__ tv1z,
    const float*__restrict__ tv2x,const float*__restrict__ tv2y,const float*__restrict__ tv2z,
...
{
    int lane=threadIdx.x&31;unsigned long long ln=0,lt=0;
    // Prefetch BVH root to L2 (all 4 int4s = 64 bytes)
    prefetchL2(&d_bvh4[0]);prefetchL2(&d_bvh4[1]);prefetchL2(&d_bvh4[2]);prefetchL2(&d_bvh4[3]);
    while(true){int bs;if(lane==0)bs=atomicAdd(&g_rayCounter2,32);bs=__shfl_sync(0xFFFFFFFF,bs,0);
        if(bs>=numRays)break;int ri=bs+lane;if(ri>=numRays)continue;
        // ★ STREAMING LOADS for ray data — bypass L2, go HBM2→L1 directly
        float ox=ldcs(&rox[ri]),oy=ldcs(&roy[ri]),oz=ldcs(&roz[ri]);
        float dx=ldcs(&rdx[ri]),dy=ldcs(&rdy[ri]),dz=ldcs(&rdz[ri]);
        float ix=ldcs(&rix[ri]),iy=ldcs(&riy[ri]),iz=ldcs(&riz[ri]);
        ...
        while(true){
            ...
            if(ni<=-2){int val=-(ni+2),tc=(val&7)+1,ts=val>>3;
                for(int i=0;i<tc;i++){int ti=ts+i;lt++;
                    // Triangle data: normal __ldg (benefits from L2 caching)
                    float v0x=tv0x[ti],v0y=tv0y[ti],v0z=tv0z[ti];
                    float e1x=tv1x[ti]-v0x,e1y=tv1y[ti]-v0y,e1z=tv1z[ti]-v0z;
                    float e2x=tv2x[ti]-v0x,e2y=tv2y[ti]-v0y,e2z=tv2z[ti]-v0z;
                    float hx=dy*e2z-dz*e2y,hy=dz*e2x-dx*e2z,hz=dx*e2y-dy*e2x;
                    ...
                    if(tt>0.001f&&tt<hitT){hitT=tt;hitTri=ti;hitU=u;hitV=v;}}
                if(sp>0)ni=stk[--sp];else break;continue;}
            // BVH load: __ldg (cached in L1+L2, persistent) 
            int4 bx=__ldg(&d_bvh4[ni*4]),by=__ldg(&d_bvh4[ni*4+1]),bz=__ldg(&d_bvh4[ni*4+2]),ch=__ldg(&d_bvh4[ni*4+3]);
            ln++;float mn0x,mn1x,mn2x,mn3x,mx0x,mx1x,mx2x,mx3x;
            float mn0y,mn1y,mn2y,mn3y,mx0y,mx1y,mx2y,mx3y;float mn0z,mn1z,mn2z,mn3z,mx0z,mx1z,mx2z,mx3z;
            d4h(bx.x,bx.y,mn0x,mn1x,mn2x,mn3x);d4h(bx.z,bx.w,mx0x,mx1x,mx2x,mx3x);
            ...
            cswap(dd[0],dd[1],ci[0],ci[1]);cswap(dd[2],dd[3],ci[2],ci[3]);
            cswap(dd[0],dd[2],ci[0],ci[2]);cswap(dd[1],dd[3],ci[1],ci[3]);cswap(dd[1],dd[2],ci[1],ci[2]);
            // ★ PREFETCH: pre-warm first child in L1 while pushing others to stack
            if(nh>=1&&ci[0]>=0){prefetchL1(&d_bvh4[ci[0]*4]);}
            for(int i=nh-1;i>=1&&sp<INCOHERENT_STACK;i--)stk[sp++]=ci[i];ni=ci[0];continue;}
        // ★ STREAMING STORE for hit results — bypass L2
        asm("st.global.cs.f32 [%0], %1;" :: "l"(&hits[ri].t), "f"(hitT));
        asm("st.global.cs.s32 [%0], %1;" :: "l"(&hits[ri].tri), "r"(hitTri));
        asm("st.global.cs.f32 [%0], %1;" :: "l"(&hits[ri].u), "f"(hitU));
        asm("st.global.cs.f32 [%0], %1;" :: "l"(&hits[ri].v), "f"(hitV));
    }
    atomicAdd(&stats[0],ln);atomicAdd(&stats[1],lt);}
...
// ═══ BENCHMARK (runs BOTH kernels A/B back-to-back for fair comparison) ═══
...
BR benchKern(KernelFn kern,bool useCounter2,RayAoS*rays,int numRays,int4*d_bvh4,int n4,float*d_v[9],cudaDeviceProp&prop){
    ...
    unsigned int z=0;
    cudaMemcpyToSymbol(g_rayCounter,&z,4);cudaMemcpyToSymbol(g_rayCounter2,&z,4);
...
int main(){
    printf("══════════════════════════════════════════════════════════════════════════\n");
    printf("  V14 ENGINE — HBM2-Aware L2 Residency Control\n");
    printf("  ld.global.cs for rays (bypass L2) | __ldg for BVH (persist in L2)\n");
    printf("  prefetch.global.L2 for BVH root | prefetch.global.L1 for next child\n");
    printf("══════════════════════════════════════════════════════════════════════════\n\n");

    cudaDeviceProp prop;cudaGetDeviceProperties(&prop,0);
    printf("  GPU: %s | L2: %d KB | HBM2: 900 GB/s\n\n",prop.name,prop.l2CacheSize/1024);
    ...
    for(int sceneK : {100, 500}){
        ...
        printf("  === %dK tris | %d BVH4 nodes | %.2f MB (L2=%d KB) ===\n",
            nt/1000,n4,n4*64.0/1048576,prop.l2CacheSize/1024);
        printf("  BVH/L2 ratio: %.1f%% — %s\n\n",
            n4*64.0/(prop.l2CacheSize)*100,
            n4*64<(int)prop.l2CacheSize?"fits in L2":"EXCEEDS L2");

        RayAoS*rays=(RayAoS*)malloc(numRays*sizeof(RayAoS));

        // Each test: run Baseline and HBM2 back-to-back with SAME data for fair comparison
        const char*labels[]={"PRIMARY","DIFFUSE UNSORTED","DIFFUSE MORTON"};
        for(int test=0;test<3;test++){
            ...
            BR rb=benchKern(traceBaseline,false,rays,numRays,d_bvh4,n4,d_v,prop);
            BR rh=benchKern(traceHBM2,true,rays,numRays,d_bvh4,n4,d_v,prop);
            double pct=(rh.mr-rb.mr)/rb.mr*100;
            printf("  %-20s Baseline: %6.0f  HBM2: %6.0f  (%+.1f%%)  [%.1f nd/r]\n",
                labels[test],rb.mr,rh.mr,pct,rb.ndR);
        }
        double l2peak=3100.0e9/(19.1*64)/1e6;
        printf("  L2 diffuse peak: %.0f MR/s\n",l2peak);
        printf("  ══════════════════════════════════════════\n\n");
