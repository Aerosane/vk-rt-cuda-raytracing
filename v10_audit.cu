// V10 audit: tests primary + diffuse + random rays, measures min/max/variance
// Reuses v10 engine, adds ray generators and comprehensive metrics
...
static int collapseRec(BN*bn,int bIdx,BVH4H*out,int*nOut){
    ...
    int openList[4],openCount=0;openList[openCount++]=bn[bIdx].l;openList[openCount++]=bn[bIdx].r;
    while(openCount<4){int bestI=-1;float bestSA=-1;
        for(int i=0;i<openCount;i++){if(bn[openList[i]].l!=-1){float s=saArea(bn[openList[i]].b);if(s>bestSA){bestSA=s;bestI=i;}}}
        if(bestI==-1)break;int bi=openList[bestI];openList[bestI]=bn[bi].l;openList[openCount++]=bn[bi].r;}
    int ni=(*nOut)++;BVH4H&nd=out[ni];nd.nChildren=openCount;
    for(int i=0;i<openCount;i++){int bi=openList[i];
    ...
    for(int i=openCount;i<4;i++){nd.child[i]=-1;nd.minX[i]=nd.maxX[i]=nd.minY[i]=nd.maxY[i]=nd.minZ[i]=nd.maxZ[i]=0;}
    return ni;
}
void reorderBVH4(BVH4H*src,int n4,BVH4H*dst,int*remap){
    int*stack=(int*)malloc(n4*4);int sp=0,out=0;stack[sp++]=0;
    while(sp>0){int i=stack[--sp];remap[i]=out;dst[out++]=src[i];
        for(int c=src[i].nChildren-1;c>=0;c--)if(src[i].child[c]>=0)stack[sp++]=src[i].child[c];}
...
void packBVH4GPU(BVH4H*nodes,int n4,int4*gpuData){
    ...
    for(int i=0;i<n4;i++){BVH4H&nd=nodes[i];
        ...
        for(int c=0;c<4;c++){if(nd.child[c]==-1){hMinX[c]=hMinY[c]=hMinZ[c]=0x7C00;hMaxX[c]=hMaxY[c]=hMaxZ[c]=0xFC00;}
            else{__half h;h=__float2half(nd.minX[c]-eps);memcpy(&hMinX[c],&h,2);h=__float2half(nd.minY[c]-eps);memcpy(&hMinY[c],&h,2);
                h=__float2half(nd.minZ[c]-eps);memcpy(&hMinZ[c],&h,2);h=__float2half(nd.maxX[c]+eps);memcpy(&hMaxX[c],&h,2);
                h=__float2half(nd.maxY[c]+eps);memcpy(&hMaxY[c],&h,2);h=__float2half(nd.maxZ[c]+eps);memcpy(&hMaxZ[c],&h,2);}}
        gpuData[i*4+0]=make_int4((hMinX[1]<<16)|hMinX[0],(hMinX[3]<<16)|hMinX[2],(hMaxX[1]<<16)|hMaxX[0],(hMaxX[3]<<16)|hMaxX[2]);
        gpuData[i*4+1]=make_int4((hMinY[1]<<16)|hMinY[0],(hMinY[3]<<16)|hMinY[2],(hMaxY[1]<<16)|hMaxY[0],(hMaxY[3]<<16)|hMaxY[2]);
        gpuData[i*4+2]=make_int4((hMinZ[1]<<16)|hMinZ[0],(hMinZ[3]<<16)|hMinZ[2],(hMaxZ[1]<<16)|hMaxZ[0],(hMaxZ[3]<<16)|hMaxZ[2]);
        gpuData[i*4+3]=make_int4(nd.child[0],nd.child[1],nd.child[2],nd.child[3]);}
}

__device__ __forceinline__ void decode4h(int lo,int hi,float&v0,float&v1,float&v2,float&v3){
...
__global__ void __launch_bounds__(256,4) traceV10(
    const int4*__restrict__ d_bvh4,int n4,int smemNodes,
...
{
    int smemN4=min(n4,smemNodes);
    for(int i=threadIdx.x;i<smemN4*4;i+=blockDim.x)s_bvh4[i]=d_bvh4[i];
    __syncthreads();
    int cnst=c_bvh4N,laneId=threadIdx.x&31;
    unsigned long long lnodes=0,ltris=0;
    while(true){
        int batchStart;if(laneId==0)batchStart=atomicAdd(&g_rayCounter,32);
        batchStart=__shfl_sync(0xFFFFFFFF,batchStart,0);
        if(batchStart>=numRays)break;int ri=batchStart+laneId;if(ri>=numRays)continue;
        ...
        int stack[SHORT_STACK];int sp=0,nodeIdx=0;
        while(true){
            if(nodeIdx==-1){if(sp>0)nodeIdx=stack[--sp];else break;continue;}
            if(nodeIdx<=-2){int val=-(nodeIdx+2),tc=(val&7)+1,ts=val>>3;
                for(int i=0;i<tc;i++){int ti=ts+i;ltris++;
                ...
                if(sp>0)nodeIdx=stack[--sp];else break;continue;}
            int4 bx,by,bz,ch;
            if(nodeIdx<smemN4){bx=s_bvh4[nodeIdx*4];by=s_bvh4[nodeIdx*4+1];bz=s_bvh4[nodeIdx*4+2];ch=s_bvh4[nodeIdx*4+3];}
            else if(nodeIdx<cnst){bx=c_bvh4[nodeIdx*4];by=c_bvh4[nodeIdx*4+1];bz=c_bvh4[nodeIdx*4+2];ch=c_bvh4[nodeIdx*4+3];}
            else{bx=__ldg(&d_bvh4[nodeIdx*4]);by=__ldg(&d_bvh4[nodeIdx*4+1]);bz=__ldg(&d_bvh4[nodeIdx*4+2]);ch=__ldg(&d_bvh4[nodeIdx*4+3]);}
            lnodes++;
            ...
            decode4h(bx.x,bx.y,mn0x,mn1x,mn2x,mn3x);decode4h(bx.z,bx.w,mx0x,mx1x,mx2x,mx3x);
            decode4h(by.x,by.y,mn0y,mn1y,mn2y,mn3y);decode4h(by.z,by.w,mx0y,mx1y,mx2y,mx3y);
            decode4h(bz.x,bz.y,mn0z,mn1z,mn2z,mn3z);decode4h(bz.z,bz.w,mx0z,mx1z,mx2z,mx3z);
            float tmn0,tmx0,tmn1,tmx1,tmn2,tmx2,tmn3,tmx3;
            {float t1=(mn0x-ox)*ix,t2=(mx0x-ox)*ix;tmn0=fminf(t1,t2);tmx0=fmaxf(t1,t2);
             t1=(mn0y-oy)*iy;t2=(mx0y-oy)*iy;tmn0=fmaxf(tmn0,fminf(t1,t2));tmx0=fminf(tmx0,fmaxf(t1,t2));
             t1=(mn0z-oz)*iz;t2=(mx0z-oz)*iz;tmn0=fmaxf(tmn0,fminf(t1,t2));tmx0=fminf(tmx0,fmaxf(t1,t2));}
            {float t1=(mn1x-ox)*ix,t2=(mx1x-ox)*ix;tmn1=fminf(t1,t2);tmx1=fmaxf(t1,t2);
             t1=(mn1y-oy)*iy;t2=(mx1y-oy)*iy;tmn1=fmaxf(tmn1,fminf(t1,t2));tmx1=fminf(tmx1,fmaxf(t1,t2));
             t1=(mn1z-oz)*iz;t2=(mx1z-oz)*iz;tmn1=fmaxf(tmn1,fminf(t1,t2));tmx1=fminf(tmx1,fmaxf(t1,t2));}
            {float t1=(mn2x-ox)*ix,t2=(mx2x-ox)*ix;tmn2=fminf(t1,t2);tmx2=fmaxf(t1,t2);
             t1=(mn2y-oy)*iy;t2=(mx2y-oy)*iy;tmn2=fmaxf(tmn2,fminf(t1,t2));tmx2=fminf(tmx2,fmaxf(t1,t2));
             t1=(mn2z-oz)*iz;t2=(mx2z-oz)*iz;tmn2=fmaxf(tmn2,fminf(t1,t2));tmx2=fminf(tmx2,fmaxf(t1,t2));}
            {float t1=(mn3x-ox)*ix,t2=(mx3x-ox)*ix;tmn3=fminf(t1,t2);tmx3=fmaxf(t1,t2);
             t1=(mn3y-oy)*iy;t2=(mx3y-oy)*iy;tmn3=fmaxf(tmn3,fminf(t1,t2));tmx3=fminf(tmx3,fmaxf(t1,t2));
             t1=(mn3z-oz)*iz;t2=(mx3z-oz)*iz;tmn3=fmaxf(tmn3,fminf(t1,t2));tmx3=fminf(tmx3,fmaxf(t1,t2));}
            int c0=ch.x,c1=ch.y,c2=ch.z,c3=ch.w;
            bool h0=(c0!=-1)&&(tmx0>=fmaxf(tmn0,0.f))&&(tmn0<hitT);
            bool h1=(c1!=-1)&&(tmx1>=fmaxf(tmn1,0.f))&&(tmn1<hitT);
            bool h2=(c2!=-1)&&(tmx2>=fmaxf(tmn2,0.f))&&(tmn2<hitT);
            bool h3=(c3!=-1)&&(tmx3>=fmaxf(tmn3,0.f))&&(tmn3<hitT);
            int numHits=h0+h1+h2+h3;
            if(numHits==0){if(sp>0)nodeIdx=stack[--sp];else break;continue;}
            float dist[4]={h0?tmn0:1e30f,h1?tmn1:1e30f,h2?tmn2:1e30f,h3?tmn3:1e30f};
            int cidx[4]={c0,c1,c2,c3};
            cswap(dist[0],dist[1],cidx[0],cidx[1]);cswap(dist[2],dist[3],cidx[2],cidx[3]);
            cswap(dist[0],dist[2],cidx[0],cidx[2]);cswap(dist[1],dist[3],cidx[1],cidx[3]);
            cswap(dist[1],dist[2],cidx[1],cidx[2]);
            for(int i=numHits-1;i>=1&&sp<SHORT_STACK;i--)stack[sp++]=cidx[i];
            nodeIdx=cidx[0];continue;
        }
        hits[ri].t=hitT;hits[ri].tri=hitTri;hits[ri].u=hitU;hits[ri].v=hitV;
    }
    atomicAdd(&stats[0],lnodes);atomicAdd(&stats[1],ltris);
}
...
void addBox(Tri*t,int&ti,float3 mn,float3 mx){
    ...
    addQuad(t,ti,a,b,cv,d);addQuad(t,ti,e,f,g,h);addQuad(t,ti,a,b,f,e);
    addQuad(t,ti,d,cv,g,h);addQuad(t,ti,a,d,h,e);addQuad(t,ti,b,cv,g,f);}
...
int genConference(Tri*t,int maxTris){
    ...
    for(int i=0;i<numT&&ti+12<maxTris;i++){float tx=((float)rand()/RAND_MAX)*16-8,tz=((float)rand()/RAND_MAX)*12-6;
        addBox(t,ti,{tx-1.f,0.7f,tz-.5f},{tx+1.f,.8f,tz+.5f});addBox(t,ti,{tx-.9f,0,tz-.05f},{tx-.8f,.7f,tz+.05f});addBox(t,ti,{tx+.8f,0,tz-.05f},{tx+.9f,.7f,tz+.05f});}
    ...
    while(ti+2<maxTris){float cx=((float)rand()/RAND_MAX)*16-8,cy=.8f+((float)rand()/RAND_MAX)*.3f;
        float cz=((float)rand()/RAND_MAX)*12-6,s=.05f+((float)rand()/RAND_MAX)*.1f;
...
void genPrimaryCoherent(RayAoS*r,int n){
    ...
    for(int i=0;i<n;i++){int px=i%w,py=i/w;
        float u=(2.f*px/w-1.f)*1.2f,v=(2.f*py/(n/w)-1.f)*.6f;
        r[i].o={0,2.5f,12};float3 d={u,-v,-1.5f};float l=sqrtf(d.x*d.x+d.y*d.y+d.z*d.z);
        d.x/=l;d.y/=l;d.z/=l;r[i].d=d;r[i].id={1.f/d.x,1.f/d.y,1.f/d.z};r[i].origIdx=i;}
    int tw=4;
...
// Diffuse: rays from random surface points, cosine-weighted hemisphere
void genDiffuse(RayAoS*r,int n,Tri*tris,int nt){
    srand(1337);
    for(int i=0;i<n;i++){
        int ti=rand()%nt;Tri&tr=tris[ti];
        ...
        r[i].o=o;r[i].d=d;r[i].id={1.f/d.x,1.f/d.y,1.f/d.z};r[i].origIdx=i;
    }
}

// Fully random: random origins inside scene, random directions
void genRandom(RayAoS*r,int n){
    srand(7777);
    for(int i=0;i<n;i++){
        float3 o={((float)rand()/RAND_MAX)*18-9,((float)rand()/RAND_MAX)*4+.5f,((float)rand()/RAND_MAX)*14-7};
        float th=acosf(2.f*((float)rand()/RAND_MAX)-1.f),ph=2.f*3.14159f*((float)rand()/RAND_MAX);
        float3 d={sinf(th)*cosf(ph),cosf(th),sinf(th)*sinf(ph)};
        float l=sqrtf(d.x*d.x+d.y*d.y+d.z*d.z);d.x/=l;d.y/=l;d.z/=l;
        r[i].o=o;r[i].d=d;r[i].id={1.f/d.x,1.f/d.y,1.f/d.z};r[i].origIdx=i;
    }
}
...
// ═══ BENCHMARK ═══
void runTest(const char*label,Tri*h_tris,int nt,RayAoS*h_rays,int numRays,
             int4*d_bvh4,int n4,float*d_v[9],cudaDeviceProp&prop)
{
    ...
    int smemBytes=SMEM_BVH4_NODES*4*sizeof(int4);
    cudaEvent_t t0,t1;cudaEventCreate(&t0);cudaEventCreate(&t1);

    // Warmup
    unsigned int zero=0;cudaMemcpyToSymbol(g_rayCounter,&zero,4);
    traceV10<<<nb,256,smemBytes>>>(d_bvh4,n4,SMEM_BVH4_NODES,
        ...
        d_hits,numRays,d_st);cudaDeviceSynchronize();

    // 20 timed runs for min/max/avg/stddev
    float times[20];
    for(int r=0;r<20;r++){
        cudaMemcpyToSymbol(g_rayCounter,&zero,4);cudaMemset(d_st,0,16);
        cudaEventRecord(t0);
        traceV10<<<nb,256,smemBytes>>>(d_bvh4,n4,SMEM_BVH4_NODES,
            d_v[0],d_v[1],d_v[2],d_v[3],d_v[4],d_v[5],d_v[6],d_v[7],d_v[8],
            d_ray[0],d_ray[1],d_ray[2],d_ray[3],d_ray[4],d_ray[5],d_ray[6],d_ray[7],d_ray[8],
            d_hits,numRays,d_st);
        cudaEventRecord(t1);cudaEventSynchronize(t1);cudaEventElapsedTime(&times[r],t0,t1);
    }
    ...
    // Stats
    float mn=1e9,mx=0,sum=0;
    for(int r=0;r<20;r++){if(times[r]<mn)mn=times[r];if(times[r]>mx)mx=times[r];sum+=times[r];}
    float avg=sum/20;
    float var=0;for(int r=0;r<20;r++){float d=times[r]-avg;var+=d*d;}var=sqrtf(var/20);
    double mrAvg=(double)numRays/(avg/1000.0)/1e6;
    double mrMin=(double)numRays/(mx/1000.0)/1e6; // worst time = min MR/s
    double mrMax=(double)numRays/(mn/1000.0)/1e6; // best time = max MR/s
    double ndr=(double)st[0]/numRays,tdr=(double)st[1]/numRays;
    double bpr=ndr*64+tdr*36;

    printf("  │%-8s│%7.0f│%7.0f│%7.0f│ %4.1f │%5.1f│ %4.1f│%5.1f%%│%6.0f│%5.2f%%│\n",
        label,mrMin,mrAvg,mrMax,var/avg*100,ndr,tdr,100.0*hitCount/numRays,bpr,var/avg*100);

    cudaEventDestroy(t0);cudaEventDestroy(t1);
    for(int j=0;j<9;j++){cudaFree(d_ray[j]);free(h_ray[j]);}
    cudaFree(d_hits);cudaFree(d_st);free(hh);
}

int main(){
    printf("╔══════════════════════════════════════════════════════════════════════════╗\n");
    printf("║  V10 COMPREHENSIVE AUDIT — Primary + Diffuse + Random rays             ║\n");
    printf("║  Metrics: MR/s (min/avg/max), nodes/ray, bytes/ray, hit%%, variance     ║\n");
    printf("╚══════════════════════════════════════════════════════════════════════════╝\n\n");

    cudaDeviceProp prop;cudaGetDeviceProperties(&prop,0);
    printf("  GPU: %s | %d SMs | L2: %dKB\n\n",prop.name,prop.multiProcessorCount,prop.l2CacheSize/1024);

    int numRays=4194304;
    int triTargets[]={100000,500000};
    ...
    for(int s=0;s<2;s++){
        ...
        BVH4H*h_bvh4=(BVH4H*)calloc(nc,sizeof(BVH4H));int n4=0;
        collapseRec(h_ord,0,h_bvh4,&n4);
        BVH4H*h_bvh4_ord=(BVH4H*)malloc(n4*sizeof(BVH4H));int*remap4=(int*)malloc(n4*4);
        reorderBVH4(h_bvh4,n4,h_bvh4_ord,remap4);
        int4*h_gpu=(int4*)malloc(n4*4*sizeof(int4));packBVH4GPU(h_bvh4_ord,n4,h_gpu);
        int cn4=n4<CONST_BVH4?n4:CONST_BVH4;
        cudaMemcpyToSymbol(c_bvh4,h_gpu,cn4*4*sizeof(int4));cudaMemcpyToSymbol(c_bvh4N,&cn4,4);
        int4*d_bvh4;cudaMalloc(&d_bvh4,n4*4*sizeof(int4));
        ...
        printf("  ═══ %dK tris (conference) | %d BVH4 nodes | %.2f MB ═══\n",nt/1000,n4,n4*64/(1024.f*1024.f));
        printf("  ┌────────┬───────┬───────┬───────┬──────┬─────┬─────┬─────┬──────┬──────┐\n");
        printf("  │RayType │ MinMR │ AvgMR │ MaxMR │ CV%%  │Nd/R │Tri/R│ Hit%%│ B/ray│ CV%%  │\n");
        printf("  ├────────┼───────┼───────┼───────┼──────┼─────┼─────┼─────┼──────┼──────┤\n");

        // Primary coherent
        RayAoS*h_rays=(RayAoS*)malloc(numRays*sizeof(RayAoS));
        genPrimaryCoherent(h_rays,numRays);
        runTest("Primary",h_tris,nt,h_rays,numRays,d_bvh4,n4,d_v,prop);

        // Diffuse (from surface, random hemisphere)
        genDiffuse(h_rays,numRays,h_to,nt);
        runTest("Diffuse",h_tris,nt,h_rays,numRays,d_bvh4,n4,d_v,prop);

        // Random (fully incoherent)
        genRandom(h_rays,numRays);
        runTest("Random",h_tris,nt,h_rays,numRays,d_bvh4,n4,d_v,prop);

        printf("  └────────┴───────┴───────┴───────┴──────┴─────┴─────┴─────┴──────┴──────┘\n\n");

        free(h_rays);free(h_tris);free(h_to);
        for(int j=0;j<9;j++){cudaFree(d_v[j]);free(h_v[j]);}
        cudaFree(d_bvh4);
        free(h_nodes);free(h_ord);free(remap);free(tidx);free(h_bvh4);free(h_bvh4_ord);free(remap4);free(h_gpu);
    }
