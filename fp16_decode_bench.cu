// FP16 DECODE OPTIMIZATION BENCHMARK
// Profiler found: FP16 decode = 29% of pipeline (24 individual half→float per BVH4 node)
// Fix: __half22float2 vector ops → 2× fewer instructions
//
// This file A/B tests the old d4h vs new d4h_vec in the full traversal kernel
...
// ═══ OLD DECODE: 4 individual half→float per call, 24 total per node ═══
__device__ __forceinline__ void d4h_old(int lo,int hi,float&v0,float&v1,float&v2,float&v3){
...
// ═══ NEW DECODE: __half22float2 vector ops, 12 instructions per node ═══
__device__ __forceinline__ void d4h_vec(int lo,int hi,float&v0,float&v1,float&v2,float&v3){
    // Reinterpret int as half2 (each int = two packed FP16 values)
    // lo = [child1_half | child0_half], hi = [child3_half | child2_half]
    __half2 h01 = *reinterpret_cast<const __half2*>(&lo);
    __half2 h23 = *reinterpret_cast<const __half2*>(&hi);
    float2 f01 = __half22float2(h01);  // single CVT instruction for 2 values
    float2 f23 = __half22float2(h23);  // single CVT instruction for 2 values
    v0=f01.x; v1=f01.y; v2=f23.x; v3=f23.y;
}

__device__ __forceinline__ void cswap(float&a,float&b,int&ia,int&ib){bool s=a>b;float tf=s?a:b;a=s?b:a;b=tf;int ti=s?ia:ib;ia=s?ib:ia;ib=ti;}

// ═══ KERNEL A: OLD DECODE (baseline) ═══
__global__ void __launch_bounds__(256,5) traceOldDecode(
    ...
    Hit*__restrict__ hits,int numRays,unsigned long long*__restrict__ stats)
{
    ...
    while(true){int bs;if(lane==0)bs=atomicAdd(&g_rayCounterA,32);bs=__shfl_sync(0xFFFFFFFF,bs,0);
        ...
        while(true){
            ...
            ln++;float mn0x,mn1x,mn2x,mn3x,mx0x,mx1x,mx2x,mx3x,mn0y,mn1y,mn2y,mn3y,mx0y,mx1y,mx2y,mx3y,mn0z,mn1z,mn2z,mn3z,mx0z,mx1z,mx2z,mx3z;
            // OLD: 6 calls × 4 conversions = 24 individual half→float
            d4h_old(bx.x,bx.y,mn0x,mn1x,mn2x,mn3x);d4h_old(bx.z,bx.w,mx0x,mx1x,mx2x,mx3x);
            d4h_old(by.x,by.y,mn0y,mn1y,mn2y,mn3y);d4h_old(by.z,by.w,mx0y,mx1y,mx2y,mx3y);
            d4h_old(bz.x,bz.y,mn0z,mn1z,mn2z,mn3z);d4h_old(bz.z,bz.w,mx0z,mx1z,mx2z,mx3z);
            ...
            bool h0=(c0!=-1)&&(t0x>=fmaxf(t0n,0.f))&&(t0n<hitT);bool h1=(c1!=-1)&&(t1x>=fmaxf(t1n,0.f))&&(t1n<hitT);
            bool h2=(c2!=-1)&&(t2x>=fmaxf(t2n,0.f))&&(t2n<hitT);bool h3=(c3!=-1)&&(t3x>=fmaxf(t3n,0.f))&&(t3n<hitT);
...
// ═══ KERNEL B: VECTORIZED DECODE (__half22float2) ═══
__global__ void __launch_bounds__(256,5) traceVecDecode(
    const int4*__restrict__ d_bvh4,int n4,int _u,
    const float*__restrict__ tv0x,const float*__restrict__ tv0y,const float*__restrict__ tv0z,
    const float*__restrict__ tv1x,const float*__restrict__ tv1y,const float*__restrict__ tv1z,
    const float*__restrict__ tv2x,const float*__restrict__ tv2y,const float*__restrict__ tv2z,
...
{
    int lane=threadIdx.x&31;unsigned long long ln=0,lt=0;
    while(true){int bs;if(lane==0)bs=atomicAdd(&g_rayCounterB,32);bs=__shfl_sync(0xFFFFFFFF,bs,0);
        if(bs>=numRays)break;int ri=bs+lane;if(ri>=numRays)continue;
        float ox=rox[ri],oy=roy[ri],oz=roz[ri],dx=rdx[ri],dy=rdy[ri],dz=rdz[ri],ix=rix[ri],iy=riy[ri],iz=riz[ri];
        float hitT=1e30f;int hitTri=-1;float hitU=0,hitV=0;int stk[INCOHERENT_STACK];int sp=0,ni=0;
        while(true){
            ...
            int4 bx=__ldg(&d_bvh4[ni*4]),by=__ldg(&d_bvh4[ni*4+1]),bz=__ldg(&d_bvh4[ni*4+2]),ch=__ldg(&d_bvh4[ni*4+3]);
            ln++;float mn0x,mn1x,mn2x,mn3x,mx0x,mx1x,mx2x,mx3x,mn0y,mn1y,mn2y,mn3y,mx0y,mx1y,mx2y,mx3y,mn0z,mn1z,mn2z,mn3z,mx0z,mx1z,mx2z,mx3z;
            // ★ NEW: 6 calls × 2 vector conversions = 12 __half22float2 ops (was 24)
            d4h_vec(bx.x,bx.y,mn0x,mn1x,mn2x,mn3x);d4h_vec(bx.z,bx.w,mx0x,mx1x,mx2x,mx3x);
            d4h_vec(by.x,by.y,mn0y,mn1y,mn2y,mn3y);d4h_vec(by.z,by.w,mx0y,mx1y,mx2y,mx3y);
            d4h_vec(bz.x,bz.y,mn0z,mn1z,mn2z,mn3z);d4h_vec(bz.z,bz.w,mx0z,mx1z,mx2z,mx3z);
            float t0n,t0x,t1n,t1x,t2n,t2x,t3n,t3x;
            {float a=(mn0x-ox)*ix,b=(mx0x-ox)*ix;t0n=fminf(a,b);t0x=fmaxf(a,b);a=(mn0y-oy)*iy;b=(mx0y-oy)*iy;t0n=fmaxf(t0n,fminf(a,b));t0x=fminf(t0x,fmaxf(a,b));a=(mn0z-oz)*iz;b=(mx0z-oz)*iz;t0n=fmaxf(t0n,fminf(a,b));t0x=fminf(t0x,fmaxf(a,b));}
            {float a=(mn1x-ox)*ix,b=(mx1x-ox)*ix;t1n=fminf(a,b);t1x=fmaxf(a,b);a=(mn1y-oy)*iy;b=(mx1y-oy)*iy;t1n=fmaxf(t1n,fminf(a,b));t1x=fminf(t1x,fmaxf(a,b));a=(mn1z-oz)*iz;b=(mx1z-oz)*iz;t1n=fmaxf(t1n,fminf(a,b));t1x=fminf(t1x,fmaxf(a,b));}
            {float a=(mn2x-ox)*ix,b=(mx2x-ox)*ix;t2n=fminf(a,b);t2x=fmaxf(a,b);a=(mn2y-oy)*iy;b=(mx2y-oy)*iy;t2n=fmaxf(t2n,fminf(a,b));t2x=fminf(t2x,fmaxf(a,b));a=(mn2z-oz)*iz;b=(mx2z-oz)*iz;t2n=fmaxf(t2n,fminf(a,b));t2x=fminf(t2x,fmaxf(a,b));}
...
uint32_t morton3D(float x,float y,float z,float3 mn,float3 mx){float nx=(x-mn.x)/(mx.x-mn.x+1e-7f),ny=(y-mn.y)/(mx.y-mn.y+1e-7f),nz=(z-mn.z)/(mx.z-mn.z+1e-7f);uint32_t ix=fminf(fmaxf(nx*1023.f,0.f),1023.f),iy=fminf(fmaxf(ny*1023.f,0.f),1023.f),iz=fminf(fmaxf(nz*1023.f,0.f),1023.f);return expand3(ix)|(expand3(iy)<<1)|(expand3(iz)<<2);}
...
BR benchK(KernelFn kern,bool useB,RayAoS*rays,int numRays,int4*d_bvh4,int n4,float*d_v[9],cudaDeviceProp&prop){
    ...
    cudaMemcpyToSymbol(g_rayCounterA,&z,4);cudaMemcpyToSymbol(g_rayCounterB,&z,4);
    ...
    for(int r=0;r<20;r++){
        cudaMemcpyToSymbol(g_rayCounterA,&z,4);cudaMemcpyToSymbol(g_rayCounterB,&z,4);cudaMemset(d_st,0,16);
...
int main(){
    printf("══════════════════════════════════════════════════════════════════\n");
    printf("  FP16 DECODE A/B TEST: Scalar vs Vectorized (__half22float2)\n");
    printf("  Profiler found: FP16 decode = 29%% of pipeline\n");
    printf("  Fix: 12 vector ops instead of 24 scalar = 2× decode speed\n");
    printf("══════════════════════════════════════════════════════════════════\n\n");
    ...
    for(int sceneK:{100,500}){
        ...
        for(int test=0;test<3;test++){
            ...
            // Run BOTH back-to-back (20 runs each for stability)
            BR old=benchK(traceOldDecode,false,rays,numRays,d_bvh4,n4,d_v,prop);
            BR vec=benchK(traceVecDecode,true,rays,numRays,d_bvh4,n4,d_v,prop);
            printf("  %-20s Old: %6.0f  Vec: %6.0f  %+.1f%%  (hit: %d%%/%d%%)\n",
                name,old.mr,vec.mr,(vec.mr-old.mr)/old.mr*100,old.hitPct,vec.hitPct);
        }
        printf("\n");
