/* v26 — Sign-Based AABB + FMA Bias Engine
 *
    ...
    Hit* __restrict__ hits, int numRays)
{
    ...
    while (true) {
        ...
        // Sign-based near/far selection: eliminates 6 fminf/fmaxf per child
        int nX = ix >= 0.f ? 0 : 4, fX = 4 - nX;
        int nY = iy >= 0.f ? 0 : 4, fY = 4 - nY;
        int nZ = iz >= 0.f ? 0 : 4, fZ = 4 - nZ;
        // FMA bias: replaces (bound - o) * invD with bound * invD + bias
        float bxb = -ox * ix, byb = -oy * iy, bzb = -oz * iz;
        ...
        while (sp > 0 && alive) {
            ...
            for (int c = 3; c >= 0; c--) {
                if (ch[c] == -1) continue;
                // Sign+FMA AABB: 6 FMA + 4 combine = 10 ops (was 6 SUB + 6 MUL + 10 min/max = 22)
                float tNx = __half2float(bx[nX+c]) * ix + bxb;
                float tFx = __half2float(bx[fX+c]) * ix + bxb;
                float tNy = __half2float(by[nY+c]) * iy + byb;
                float tFy = __half2float(by[fY+c]) * iy + byb;
                float tNz = __half2float(bz[nZ+c]) * iz + bzb;
                float tFz = __half2float(bz[fZ+c]) * iz + bzb;
                float tN = fmaxf(fmaxf(tNx, tNy), tNz);
                float tF = fminf(fminf(tFx, tFy), tFz);
...
int main() {
    printf("══════════════════════════════════════════════════════════════════════════\n");
    printf("  V26 — Sign+FMA AABB + Constant BVH Cache + 4M Rays\n");
