// ═══════════════════════════════════════════════════════════════════════════
// V15 — WAVE-COMPACTED ADAPTIVE ENGINE
// 
// Synthesis of ALL findings:
//   - v10: persistent threads + short stack (proven best base)
//   - v12: __ldg only, no smem (Volta 128KB L1 > smem)
//   - v11: warp-coherent HURTS (100% eff but 24× node visits)
//   - Profiler: memory-latency bound, not compute bound
//   - Warp eff: Diffuse 35.1% → 65% threads idle
//   - Compaction: 1.8×-2.5× at low survival
//   - v14: streaming loads = no help (BVH fits L2)
//
// Strategy: WAVE TRAVERSAL with intra-warp compaction
//   1. Each warp processes 32 rays independently
//   2. When a ray terminates, it gets a NEW ray from global queue
//   3. Use __ballot_sync to detect terminated rays, __popc for count
//   4. Terminated lanes fetch new rays → keeps warp fully occupied
//   5. No lockstep (avoids v11's 24× node explosion)
//   6. Key insight: high occupancy (48 regs, 40 warps/SM) hides latency
//      while wave refill maintains warp utilization
//
// Expected: +30-50% diffuse (35%→65%+ warp efficiency)
// ═══════════════════════════════════════════════════════════════════════════
...
// Two persistent ray counters: one per kernel
...
int buildSAH(BN*n,Tri*t,int*idx,int&nc,int s,int c,int d){
    ...
    for(int i=1;i<c;i++){float3 cc=triCen(t[idx[s+i]]);cb.bmin={fminf(cb.bmin.x,cc.x),fminf(cb.bmin.y,cc.y),fminf(cb.bmin.z,cc.z)};cb.bmax={fmaxf(cb.bmax.x,cc.x),fmaxf(cb.bmax.y,cc.y),fmaxf(cb.bmax.z,cc.z)};}
    float bc=FLT_MAX;int ba=-1,bb=-1;float ps=saArea(bounds);
    for(int ax=0;ax<3;ax++){float amn=ax==0?cb.bmin.x:ax==1?cb.bmin.y:cb.bmin.z,amx=ax==0?cb.bmax.x:ax==1?cb.bmax.y:cb.bmax.z;
        if(amx-amn<1e-7f)continue;struct B{AABB b;int c;};B bins[SAH_BINS];
        ...
        for(int i=0;i<c;i++){float3 cc=triCen(t[idx[s+i]]);float cv=ax==0?cc.x:ax==1?cc.y:cc.z;int b2=fminf(fmaxf((int)((cv-amn)*sc),0),SAH_BINS-1);bins[b2].b=unite(bins[b2].b,triBox(t[idx[s+i]]));bins[b2].c++;}
        ...
        for(int i=0;i<SAH_BINS-1;i++){if(lC[i]==0||rC[i+1]==0)continue;float cost=1.0f+(lC[i]*saArea(lB[i])+rC[i+1]*saArea(rB[i+1]))/ps;if(cost<bc){bc=cost;ba=ax;bb=i;}}}
    ...
    int i=s,j=s+c-1;while(i<=j){float3 cc=triCen(t[idx[i]]);float cv=ba==0?cc.x:ba==1?cc.y:cc.z;int b2=fminf(fmaxf((int)((cv-amn)*sc),0),SAH_BINS-1);if(b2<=bb)i++;else{int tmp=idx[i];idx[i]=idx[j];idx[j]=tmp;j--;}}
...
static int collapseRec(BN*bn,int bIdx,BVH4H*out,int*nOut){
    ...
    int ni=(*nOut)++;BVH4H&nd=out[ni];nd.nChildren=oc;for(int i=0;i<oc;i++){int bi=ol[i];nd.minX[i]=bn[bi].b.bmin.x;nd.minY[i]=bn[bi].b.bmin.y;nd.minZ[i]=bn[bi].b.bmin.z;nd.maxX[i]=bn[bi].b.bmax.x;nd.maxY[i]=bn[bi].b.bmax.y;nd.maxZ[i]=bn[bi].b.bmax.z;if(bn[bi].l==-1)nd.child[i]=-((bn[bi].ts<<3)|(bn[bi].tc-1))-2;else nd.child[i]=collapseRec(bn,bi,out,nOut);}
    for(int i=oc;i<4;i++){nd.child[i]=-1;nd.minX[i]=nd.maxX[i]=nd.minY[i]=nd.maxY[i]=nd.minZ[i]=nd.maxZ[i]=0;}return ni;}
...
// ═══════════════════════════════════════════════════════════════════
// KERNEL A: BASELINE (v12 Incoherent — proven best)
// Each thread traces independently, fetches 32 rays at a time per warp
// ═══════════════════════════════════════════════════════════════════
    ...
    Hit*__restrict__ hits,int numRays,unsigned long long*__restrict__ stats)
{
    ...
    while(true){
        ...
        if(bs>=numRays)break;
        int ri=bs+lane;if(ri>=numRays)continue;
        ...
        while(true){
            ...
            for(int i=nh-1;i>=1&&sp<STACK_DEPTH;i--)stk[sp++]=ci[i];
            ni=ci[0];}
        hits[ri].t=hitT;hits[ri].tri=hitTri;hits[ri].u=hitU;hits[ri].v=hitV;}
}

// ═══════════════════════════════════════════════════════════════════
// KERNEL B: WAVE-REFILL — terminated lanes get new rays immediately
// 
// Key differences from baseline:
//   1. Per-lane state tracking (active/done)
//   2. When a lane finishes, it writes result and gets a new ray
//   3. Uses __ballot_sync to find finished lanes
//   4. Global atomic for individual lane refill (not 32-at-a-time)
//   5. Warp stays busy until ALL global rays exhausted
// ═══════════════════════════════════════════════════════════════════
__global__ void __launch_bounds__(256,5) traceWaveRefill(
    const int4*__restrict__ d_bvh4,int n4,int _u,
    const float*__restrict__ tv0x,const float*__restrict__ tv0y,const float*__restrict__ tv0z,
    const float*__restrict__ tv1x,const float*__restrict__ tv1y,const float*__restrict__ tv1z,
    const float*__restrict__ tv2x,const float*__restrict__ tv2y,const float*__restrict__ tv2z,
    ...
    Hit*__restrict__ hits,int numRays,unsigned long long*__restrict__ stats)
{
    ...
    // Each lane maintains its own ray state
    float ox,oy,oz,dx2,dy2,dz2,ix2,iy2,iz2;
    float hitT;int hitTri;float hitU,hitV;
    int stk[STACK_DEPTH];
    int sp,ni;
    int ri=-1;    // current ray index (-1 = need new ray)
    bool active=false;
    
    // Initial ray fetch: warp-coherent batch of 32
    {
        int bs;
        if(lane==0) bs=atomicAdd(&g_rayCounterB,32);
        bs=__shfl_sync(0xFFFFFFFF,bs,0);
        ri=bs+lane;
        if(ri<numRays){
            ox=rox[ri];oy=roy[ri];oz=roz[ri];
            dx2=rdx[ri];dy2=rdy[ri];dz2=rdz[ri];
            ix2=rix[ri];iy2=riy[ri];iz2=riz[ri];
            hitT=1e30f;hitTri=-1;hitU=hitV=0;sp=0;ni=0;
            active=true;
        }
    }
    
    while(true){
        // Check if any lane in warp is still active
        unsigned int activeMask=__ballot_sync(0xFFFFFFFF,active);
        if(activeMask==0) break;  // entire warp done
        
        if(!active){
            // This lane is idle — try to get a new ray
            // Use warp-level atomic: only lane 0 fetches, distributes via shuffle
            unsigned int idleMask=__ballot_sync(0xFFFFFFFF,!active && (activeMask!=0));
            int idleCount=__popc(idleMask);
            if(idleCount>0){
                int newBase;
                if(lane==0) newBase=atomicAdd(&g_rayCounterB,idleCount);
                newBase=__shfl_sync(0xFFFFFFFF,newBase,0);
                // Compute this lane's offset among idle lanes
                unsigned int lowerMask=idleMask&((1u<<lane)-1);
                int myOffset=__popc(lowerMask);
                if(!active){
                    ri=newBase+myOffset;
                    if(ri<numRays){
                        ox=rox[ri];oy=roy[ri];oz=roz[ri];
            ...
            if(!active){
                // Still no ray — sync and continue (other lanes may still be active)
                // Do a dummy traversal step to avoid infinite busy-wait
                continue;
            }
        }
        
        // ═══ SINGLE TRAVERSAL STEP (not full ray!) ═══
        // Process ONE node per iteration, allowing check for idle lanes frequently
        if(ni==-1){
            if(sp>0){ni=stk[--sp];}
            else{
                // Ray complete — write result
                hits[ri].t=hitT;hits[ri].tri=hitTri;hits[ri].u=hitU;hits[ri].v=hitV;
                active=false;
                continue;
            }
        ...
        if(ni<=-2){
            ...
            for(int i=0;i<tc;i++){int ti=ts+i;
                ...
                if(tt>0.001f&&tt<hitT){hitT=tt;hitTri=ti;hitU=u;hitV=v;}}
            if(sp>0)ni=stk[--sp];
            else{hits[ri].t=hitT;hits[ri].tri=hitTri;hits[ri].u=hitU;hits[ri].v=hitV;active=false;}
            continue;
        }
        
        // Internal node — AABB test
        int4 bx=__ldg(&d_bvh4[ni*4]),by=__ldg(&d_bvh4[ni*4+1]),bz=__ldg(&d_bvh4[ni*4+2]),ch=__ldg(&d_bvh4[ni*4+3]);
        float mn0x,mn1x,mn2x,mn3x,mx0x,mx1x,mx2x,mx3x;
        float mn0y,mn1y,mn2y,mn3y,mx0y,mx1y,mx2y,mx3y;
        float mn0z,mn1z,mn2z,mn3z,mx0z,mx1z,mx2z,mx3z;
        ...
        d4h(bz.x,bz.y,mn0z,mn1z,mn2z,mn3z);d4h(bz.z,bz.w,mx0z,mx1z,mx2z,mx3z);
        float t0n,t0x2,t1n,t1x2,t2n,t2x2,t3n,t3x2;
        {float a=(mn0x-ox)*ix2,b=(mx0x-ox)*ix2;t0n=fminf(a,b);t0x2=fmaxf(a,b);a=(mn0y-oy)*iy2;b=(mx0y-oy)*iy2;t0n=fmaxf(t0n,fminf(a,b));t0x2=fminf(t0x2,fmaxf(a,b));a=(mn0z-oz)*iz2;b=(mx0z-oz)*iz2;t0n=fmaxf(t0n,fminf(a,b));t0x2=fminf(t0x2,fmaxf(a,b));}
        {float a=(mn1x-ox)*ix2,b=(mx1x-ox)*ix2;t1n=fminf(a,b);t1x2=fmaxf(a,b);a=(mn1y-oy)*iy2;b=(mx1y-oy)*iy2;t1n=fmaxf(t1n,fminf(a,b));t1x2=fminf(t1x2,fmaxf(a,b));a=(mn1z-oz)*iz2;b=(mx1z-oz)*iz2;t1n=fmaxf(t1n,fminf(a,b));t1x2=fminf(t1x2,fmaxf(a,b));}
        {float a=(mn2x-ox)*ix2,b=(mx2x-ox)*ix2;t2n=fminf(a,b);t2x2=fmaxf(a,b);a=(mn2y-oy)*iy2;b=(mx2y-oy)*iy2;t2n=fmaxf(t2n,fminf(a,b));t2x2=fminf(t2x2,fmaxf(a,b));a=(mn2z-oz)*iz2;b=(mx2z-oz)*iz2;t2n=fmaxf(t2n,fminf(a,b));t2x2=fminf(t2x2,fmaxf(a,b));}
        {float a=(mn3x-ox)*ix2,b=(mx3x-ox)*ix2;t3n=fminf(a,b);t3x2=fmaxf(a,b);a=(mn3y-oy)*iy2;b=(mx3y-oy)*iy2;t3n=fmaxf(t3n,fminf(a,b));t3x2=fminf(t3x2,fmaxf(a,b));a=(mn3z-oz)*iz2;b=(mx3z-oz)*iz2;t3n=fmaxf(t3n,fminf(a,b));t3x2=fminf(t3x2,fmaxf(a,b));}
        int c0=ch.x,c1=ch.y,c2=ch.z,c3=ch.w;
        bool h0=(c0!=-1)&&(t0x2>=fmaxf(t0n,0.f))&&(t0n<hitT);
        bool h1=(c1!=-1)&&(t1x2>=fmaxf(t1n,0.f))&&(t1n<hitT);
        bool h2=(c2!=-1)&&(t2x2>=fmaxf(t2n,0.f))&&(t2n<hitT);
...
int main(){
    printf("══════════════════════════════════════════════════════════════════════════\n");
    printf("  V15 ENGINE — Wave-Refill Adaptive Traversal\n");
    printf("  Terminated lanes get new rays immediately via warp-level ballot+refill\n");
    printf("  Target: +30-50%% diffuse via 35%%→65%%+ warp efficiency\n");
    printf("══════════════════════════════════════════════════════════════════════════\n\n");
    ...
    for(int sceneK:{100,500}){
        ...
        for(int test=0;test<3;test++){
            ...
            BR wave=benchK(traceWaveRefill,true,rays,numRays,d_bvh4,n4,d_v,prop);
            double delta=(wave.mr-base.mr)/base.mr*100;
            printf("  %-20s Base: %6.0f  Wave: %6.0f  %+.1f%%  (hit: %d%%/%d%%)\n",
                name,base.mr,wave.mr,delta,base.hitPct,wave.hitPct);
        }
        printf("\n");
