// V10: Hardware-Scheduled + Dedicated RT Cache + Ultra-Low Latency BVH4
// Three pillars over v9:
//   1. HARDWARE SCHEDULING: Persistent kernel + warp work-stealing (atomicAdd batch)
//   2. DEDICATED RT CACHE: Shared-memory L0 BVH cache (20cy) → const (4cy broadcast) → __ldg (80cy tex L1)
//   3. ULTRA-LOW LATENCY: Short stack[12] → (256,4) occupancy = 1024 threads/SM (+33%)
//                         Branchless 4-child sort, speculative prefetch
// v9 baseline: 3277/2841/2597/2201 MR/s at 49K/99K/199K/499K (8.4-9.6 nd/ray)
...
#define SMEM_BVH4_NODES 128  // top 128 BVH4 nodes in shared memory = 8KB

#define CONST_BVH4 1023
__constant__ int4 c_bvh4[CONST_BVH4 * 4];
__constant__ int c_bvh4N;

// Persistent kernel ray counter
...
static int collapseRec(BN*bn, int bIdx, BVH4H*out, int*nOut) {
    if(bn[bIdx].l == -1) {
        int ni = (*nOut)++;
        BVH4H&nd = out[ni]; nd.nChildren = 1;
        ...
        for(int i=1;i<4;i++) nd.child[i] = -1;
        return ni;
    }
    int openList[4], openCount = 0;
    openList[openCount++] = bn[bIdx].l;
    openList[openCount++] = bn[bIdx].r;
    while(openCount < 4) {
        int bestI = -1; float bestSA = -1;
        for(int i = 0; i < openCount; i++) {
            if(bn[openList[i]].l != -1) {
                float s = saArea(bn[openList[i]].b);
                if(s > bestSA) { bestSA = s; bestI = i; }
            }
        }
        if(bestI == -1) break;
        int bi = openList[bestI];
        openList[bestI] = bn[bi].l;
        openList[openCount++] = bn[bi].r;
    }
...
void packBVH4GPU(BVH4H*nodes, int n4, int4*gpuData) {
    ...
    for(int i = 0; i < n4; i++) {
        ...
        for(int c = 0; c < 4; c++) {
            if(nd.child[c] == -1) {
                hMinX[c]=hMinY[c]=hMinZ[c]=0x7C00;
                hMaxX[c]=hMaxY[c]=hMaxZ[c]=0xFC00;
            } else {
                __half h;
        ...
        gpuData[i*4+0]=make_int4((hMinX[1]<<16)|hMinX[0],(hMinX[3]<<16)|hMinX[2],
            (hMaxX[1]<<16)|hMaxX[0],(hMaxX[3]<<16)|hMaxX[2]);
        gpuData[i*4+1]=make_int4((hMinY[1]<<16)|hMinY[0],(hMinY[3]<<16)|hMinY[2],
            (hMaxY[1]<<16)|hMaxY[0],(hMaxY[3]<<16)|hMaxY[2]);
        gpuData[i*4+2]=make_int4((hMinZ[1]<<16)|hMinZ[0],(hMinZ[3]<<16)|hMinZ[2],
...
// Branchless compare-swap for sorting network
__device__ __forceinline__ void cswap(float&a, float&b, int&ia, int&ib) {
    bool s = a > b;
    float tf = s ? a : b; a = s ? b : a; b = tf;
    int ti = s ? ia : ib; ia = s ? ib : ia; ib = ti;
}

// ═══ V10 KERNEL: Persistent + Smem Cache + Short Stack ═══
...
{
    // ─── PILLAR 2: Load top BVH4 nodes into shared memory ───
    int smemN4 = min(n4, smemNodes);
    for(int i = threadIdx.x; i < smemN4 * 4; i += blockDim.x)
        s_bvh4[i] = d_bvh4[i];
    __syncthreads();
    ...
    // ─── PILLAR 1: Persistent warp work-stealing ───
    while(true) {
        // Warp leader grabs batch of 32 rays
        int batchStart;
        if(laneId == 0) batchStart = atomicAdd(&g_rayCounter, 32);
        batchStart = __shfl_sync(0xFFFFFFFF, batchStart, 0);
        if(batchStart >= numRays) break;
        int ri = batchStart + laneId;
        ...
        // ─── PILLAR 3: Short stack with restart trail ───
        int stack[SHORT_STACK]; int sp=0;
        int nodeIdx = 0;
        ...
        while(true) {
            ...
            if(nodeIdx <= -2) {
                // LEAF: Möller-Trumbore intersection
                int val = -(nodeIdx + 2);
                int tc = (val & 7) + 1, ts = val >> 3;
                for(int i=0; i<tc; i++) {
                    int ti=ts+i; ltris++;
            ...
            // INTERNAL: Load BVH4 node from tiered cache
            int4 bx, by, bz, ch;
            if(nodeIdx < smemN4) {
                // L0: Shared memory (~20 cycles, zero contention for broadcast reads)
                bx = s_bvh4[nodeIdx*4+0]; by = s_bvh4[nodeIdx*4+1];
                bz = s_bvh4[nodeIdx*4+2]; ch = s_bvh4[nodeIdx*4+3];
            } else if(nodeIdx < cnst) {
                // L1: Constant memory (4 cycles broadcast, 64KB)
                bx = c_bvh4[nodeIdx*4+0]; by = c_bvh4[nodeIdx*4+1];
                bz = c_bvh4[nodeIdx*4+2]; ch = c_bvh4[nodeIdx*4+3];
            } else {
                // L2: __ldg through texture L1 cache (~80 cycles)
                bx = __ldg(&d_bvh4[nodeIdx*4+0]); by = __ldg(&d_bvh4[nodeIdx*4+1]);
                bz = __ldg(&d_bvh4[nodeIdx*4+2]); ch = __ldg(&d_bvh4[nodeIdx*4+3]);
            }
            lnodes++;

            // Decode all child bounds from FP16
            ...
            decode4h(bx.x,bx.y, mn0x,mn1x,mn2x,mn3x);
            decode4h(bx.z,bx.w, mx0x,mx1x,mx2x,mx3x);
            decode4h(by.x,by.y, mn0y,mn1y,mn2y,mn3y);
            decode4h(by.z,by.w, mx0y,mx1y,mx2y,mx3y);
            decode4h(bz.x,bz.y, mn0z,mn1z,mn2z,mn3z);
            decode4h(bz.z,bz.w, mx0z,mx1z,mx2z,mx3z);

            // 4 AABB slab tests — fully unrolled, no branching
            float tmn0,tmx0,tmn1,tmx1,tmn2,tmx2,tmn3,tmx3;

            // Child 0
            { float t1=(mn0x-ox)*ix,t2=(mx0x-ox)*ix;
              tmn0=fminf(t1,t2);tmx0=fmaxf(t1,t2);
              t1=(mn0y-oy)*iy;t2=(mx0y-oy)*iy;
              tmn0=fmaxf(tmn0,fminf(t1,t2));tmx0=fminf(tmx0,fmaxf(t1,t2));
              t1=(mn0z-oz)*iz;t2=(mx0z-oz)*iz;
              tmn0=fmaxf(tmn0,fminf(t1,t2));tmx0=fminf(tmx0,fmaxf(t1,t2)); }
            // Child 1
            { float t1=(mn1x-ox)*ix,t2=(mx1x-ox)*ix;
              tmn1=fminf(t1,t2);tmx1=fmaxf(t1,t2);
              t1=(mn1y-oy)*iy;t2=(mx1y-oy)*iy;
              tmn1=fmaxf(tmn1,fminf(t1,t2));tmx1=fminf(tmx1,fmaxf(t1,t2));
              t1=(mn1z-oz)*iz;t2=(mx1z-oz)*iz;
              tmn1=fmaxf(tmn1,fminf(t1,t2));tmx1=fminf(tmx1,fmaxf(t1,t2)); }
            // Child 2
            { float t1=(mn2x-ox)*ix,t2=(mx2x-ox)*ix;
              tmn2=fminf(t1,t2);tmx2=fmaxf(t1,t2);
              t1=(mn2y-oy)*iy;t2=(mx2y-oy)*iy;
              tmn2=fmaxf(tmn2,fminf(t1,t2));tmx2=fminf(tmx2,fmaxf(t1,t2));
              t1=(mn2z-oz)*iz;t2=(mx2z-oz)*iz;
              tmn2=fmaxf(tmn2,fminf(t1,t2));tmx2=fminf(tmx2,fmaxf(t1,t2)); }
            // Child 3
            { float t1=(mn3x-ox)*ix,t2=(mx3x-ox)*ix;
              tmn3=fminf(t1,t2);tmx3=fmaxf(t1,t2);
              t1=(mn3y-oy)*iy;t2=(mx3y-oy)*iy;
              tmn3=fmaxf(tmn3,fminf(t1,t2));tmx3=fminf(tmx3,fmaxf(t1,t2));
              t1=(mn3z-oz)*iz;t2=(mx3z-oz)*iz;
              tmn3=fmaxf(tmn3,fminf(t1,t2));tmx3=fminf(tmx3,fmaxf(t1,t2)); }

            // Hit testing (branchless flags)
            ...
            // Compact + sort hits (branchless sorting network for 4 elements)
            float dist[4] = {h0?tmn0:1e30f, h1?tmn1:1e30f, h2?tmn2:1e30f, h3?tmn3:1e30f};
            int cidx[4] = {c0, c1, c2, c3};

            // 5-comparator sorting network for 4 elements (optimal)
            cswap(dist[0],dist[1],cidx[0],cidx[1]);
            cswap(dist[2],dist[3],cidx[2],cidx[3]);
            cswap(dist[0],dist[2],cidx[0],cidx[2]);
            cswap(dist[1],dist[3],cidx[1],cidx[3]);
            cswap(dist[1],dist[2],cidx[1],cidx[2]);

            // Push far children, traverse nearest (sorted ascending, so [0] is nearest)
            // Short stack: push up to SHORT_STACK entries, drop farthest if full
            for(int i=numHits-1; i>=1 && sp<SHORT_STACK; i--)
                stack[sp++] = cidx[i];
            nodeIdx = cidx[0];
            continue;
        }
...
// ═══ V9 REFERENCE KERNEL (for side-by-side comparison) ═══
__global__ void __launch_bounds__(256,3) traceV9ref(
    ...
    Hit*__restrict__ hits, int numRays, unsigned long long*__restrict__ stats)
{
    ...
    for(int ri=gid; ri<numRays; ri+=stride) {
        ...
        float ix=rix[ri],iy=riy[ri],iz=riz[ri];
        float hitT=1e30f;int hitTri=-1;float hitU=0,hitV=0;
        int stack[32];int sp=0,nodeIdx=0;
        while(true){
            if(nodeIdx==-1){if(sp>0)nodeIdx=stack[--sp];else break;continue;}
            if(nodeIdx<=-2){
                int val=-(nodeIdx+2),tc=(val&7)+1,ts=val>>3;
            ...
            int4 bx,by,bz,ch_v;
            if(nodeIdx<cnst){bx=c_bvh4[nodeIdx*4];by=c_bvh4[nodeIdx*4+1];bz=c_bvh4[nodeIdx*4+2];ch_v=c_bvh4[nodeIdx*4+3];}
            else{bx=__ldg(&d_bvh4[nodeIdx*4]);by=__ldg(&d_bvh4[nodeIdx*4+1]);bz=__ldg(&d_bvh4[nodeIdx*4+2]);ch_v=__ldg(&d_bvh4[nodeIdx*4+3]);}
            lnodes++;
            ...
            int cChild[4]={ch_v.x,ch_v.y,ch_v.z,ch_v.w};
            float tMinArr[4];int hitMask=0;
            for(int c=0;c<4;c++){if(cChild[c]==-1)continue;
                ...
                if(tmx>=fmaxf(tmn,0.0f)&&tmn<hitT){tMinArr[c]=tmn;hitMask|=(1<<c);}}
            int numHits=__popc(hitMask);
            if(numHits==0){if(sp>0)nodeIdx=stack[--sp];else break;continue;}
            int hIdx[4];float hDist[4];int h=0;
            for(int c=0;c<4;c++)if(hitMask&(1<<c)){hIdx[h]=cChild[c];hDist[h]=tMinArr[c];h++;}
            for(int i=1;i<numHits;i++){float key=hDist[i];int ki=hIdx[i];int j=i-1;
              ...
              cudaDeviceProp&prop)
{
    ...
    BVH4H*h_bvh4=(BVH4H*)calloc(nc, sizeof(BVH4H));
    int n4=0; collapseRec(h_ord, 0, h_bvh4, &n4);
    ...
    int smemBytes = SMEM_BVH4_NODES * 4 * sizeof(int4); // 128 nodes × 64B = 8192 bytes

    // ─── V9 REFERENCE ───
    int nb9 = prop.multiProcessorCount * 6;
    cudaMemset(d_st,0,16);
    traceV9ref<<<nb9,256>>>(d_bvh4,n4,
    ...
    for(int r=0;r<10;r++){
        ...
        traceV9ref<<<nb9,256>>>(d_bvh4,n4,
            ...
            d_ray[0],d_ray[1],d_ray[2],d_ray[3],d_ray[4],d_ray[5],d_ray[6],d_ray[7],d_ray[8],
            d_hits,numRays,d_st);
        cudaEventRecord(t1);cudaEventSynchronize(t1);float ms;cudaEventElapsedTime(&ms,t0,t1);totalV9+=ms;}
    float avgV9=totalV9/10; double mrV9=(double)numRays/(avgV9/1000.0)/1e6;

    // Verify v9 hits
    Hit*hh=(Hit*)malloc(numRays*sizeof(Hit));
    cudaMemcpy(hh,d_hits,numRays*sizeof(Hit),cudaMemcpyDeviceToHost);
    int hcV9=0; for(int i=0;i<numRays;i++) if(hh[i].tri>=0) hcV9++;

    // ─── V10 PERSISTENT + SMEM + SHORT STACK ───
    int nb10 = prop.multiProcessorCount * 4; // match occupancy ceiling
    // Reset persistent counter
    unsigned int zero = 0;
    cudaMemcpyToSymbol(g_rayCounter, &zero, sizeof(unsigned int));
    // Warmup
    traceV10<<<nb10,256,smemBytes>>>(d_bvh4,n4,SMEM_BVH4_NODES,
        d_v[0],d_v[1],d_v[2],d_v[3],d_v[4],d_v[5],d_v[6],d_v[7],d_v[8],
        d_ray[0],d_ray[1],d_ray[2],d_ray[3],d_ray[4],d_ray[5],d_ray[6],d_ray[7],d_ray[8],
        d_hits,numRays,d_st);
    cudaDeviceSynchronize();
    ...
    for(int r=0;r<10;r++){
        ...
        float ms; cudaEventElapsedTime(&ms,t0,t1); totalV10+=ms;
    }
    float avgV10=totalV10/10; double mrV10=(double)numRays/(avgV10/1000.0)/1e6;
    unsigned long long stV10[2]; cudaMemcpy(stV10,d_st,16,cudaMemcpyDeviceToHost);

    cudaMemcpy(hh,d_hits,numRays*sizeof(Hit),cudaMemcpyDeviceToHost);
    int hcV10=0; for(int i=0;i<numRays;i++) if(hh[i].tri>=0) hcV10++;

    double ndr=(double)stV10[0]/numRays;
    double tdr=(double)stV10[1]/numRays;
    double bpr=ndr*64+tdr*36;
    float bvhMB=n4*64/(1024.0f*1024.0f);
    double speedup=mrV10/mrV9*100-100;

    printf("  │%5dK│%-7s│%7.1f│%7.1f│%+6.1f%%│%5.1f│ %4.1f│%5.1f%%│%6.0f│%5.2f│\n",
        nt/1000, label, mrV9, mrV10, speedup, ndr, tdr,
        100.0*hcV10/numRays, bpr, bvhMB);

    // Verify correctness: hit counts must match
    if(hcV10 != hcV9) printf("  ⚠ HIT MISMATCH: v9=%d v10=%d\n", hcV9, hcV10);
...
int main(){
    printf("╔═══════════════════════════════════════════════════════════════════════════════════════╗\n");
    printf("║  V100 RT Engine v10 — Hardware Scheduling + Dedicated RT Cache + Ultra-Low Latency   ║\n");
    printf("║  Pillar 1: Persistent kernel + warp work-stealing (atomic batch of 32 rays)          ║\n");
    printf("║  Pillar 2: 3-tier BVH cache: smem(20cy) → const(4cy) → __ldg(80cy tex L1)           ║\n");
    printf("║  Pillar 3: Short stack[12] → (256,4) = 1024 threads/SM (+33%% occupancy)             ║\n");
    printf("║            Branchless sorting network + fully unrolled slab tests                     ║\n");
    printf("╚═══════════════════════════════════════════════════════════════════════════════════════╝\n\n");

    cudaDeviceProp prop; cudaGetDeviceProperties(&prop,0);
    printf("  GPU: %s | %d SMs | L2: %dKB\n", prop.name, prop.multiProcessorCount, prop.l2CacheSize/1024);
    printf("  v9 ref: (256,3)=768 thr/SM, grid-stride, stack[32], no smem cache\n");
    printf("  v10:    (256,4)=1024 thr/SM, persistent+atomic, stack[12], smem L0 cache\n\n");

    int numRays = 4194304;
    int triTargets[] = {50000, 100000, 200000, 500000};

    printf("  ┌─────┬───────┬───────┬───────┬──────┬─────┬─────┬─────┬──────┬─────┐\n");
    printf("  │Tris │Scene  │ v9ref │ v10   │  Δ   │Nd/R │Tri/R│ Hit%%│ B/ray│BVH4M│\n");
    printf("  ├─────┼───────┼───────┼───────┼──────┼─────┼─────┼─────┼──────┼─────┤\n");
    ...
    printf("  └─────┴───────┴───────┴───────┴──────┴─────┴─────┴─────┴──────┴─────┘\n\n");
    printf("  v7=1944 │ v8=3142 │ v9=2841 │ v10=? MR/s @ 99K conference primary\n");
