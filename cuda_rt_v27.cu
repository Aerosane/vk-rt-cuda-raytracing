/* v27 — Inline Ray Gen + Zero-HBM Primary
 *
 * KEY IMPROVEMENTS OVER v22:
 * 1) Constant memory top BVH nodes (1023 nodes in 64KB constant cache).
 *    Every ray hits top levels → broadcast from constant cache = 1 cycle
 *    vs L2 ~200 cycles. THIS was missing from v22, explains -20% primary.
 *
 * 2) Fused warp-level compaction: atomicAdd + __ballot_sync inside
 *    wavefrontStep kernel. Eliminates CUB launch + D2H sync per iteration.
 *    Each iteration: 1 kernel instead of 3 ops (kernel + CUB + memcpy).
 *
 * 3) 4M rays (matches v12 for fair comparison).
 *
 * RETAINED FROM v22:
 * - Wavefront decomposition (fixed N-step + compaction)
 * - Branchless sorting network (5 predicated compare-swaps)
 * - First-hit BVH probe sort (top-2-level subtree key)
 * - NoSort megakernel for primary
 * - Binned SAH for fast BVH construction
 */
...
// ======================== PRIMARY INLINE: Zero-HBM Ray Generation ========================
__global__ void __launch_bounds__(256, 5) tracePrimaryInline(
    const int4* __restrict__ bvh, int n4,
    const float* __restrict__ tv0x, const float* __restrict__ tv0y, const float* __restrict__ tv0z,
    const float* __restrict__ tv1x, const float* __restrict__ tv1y, const float* __restrict__ tv1z,
    const float* __restrict__ tv2x, const float* __restrict__ tv2y, const float* __restrict__ tv2z,
    ...
    float camOx, float camOy, float camOz)
{
    ...
    while (true) {
        ...
        while (sp > 0 && alive) {
            ...
            const __half* bx = (const __half*)&n0, *by = (const __half*)&n1, *bz = (const __half*)&n2;
            const int* ch = (const int*)&n3;
            // Test all 4 children, record distances
            ...
            // Branchless CSWAP sorting network (5 compare-swaps, zero SIMT divergence)
            ...
            #undef CSWAP_P
            // Push in reverse sorted order (closest on top of stack)
            for (int c = 3; c >= 0; c--)
    ...
    int* __restrict__ sortKeys, int numRays)
{
    ...
    // Traverse 2 levels of BVH4 to find entry subtree (always top nodes → use constant mem)
    int key = 0;
    for (int level = 0; level < 2; level++) {
        int4 n0, n1, n2, n3;
        loadBVH4Node(bvh, key, n0, n1, n2, n3);
        const __half* bx = (const __half*)&n0, *by = (const __half*)&n1, *bz = (const __half*)&n2;
        const int* ch = (const int*)&n3;
...
// ======================== Wavefront Step: N-step traversal with fused compaction ========================
__global__ void __launch_bounds__(256, 4) wavefrontStep(
    ...
    // Active ray indirection (input + output for fused compaction)
    const int* __restrict__ activeMapIn, int numActive,
    int* __restrict__ activeMapOut, int* __restrict__ outCount,
    ...
    unsigned long long* __restrict__ warpEff)
{
    ...
    for (int step = 0; step < STEPS_PER_LAUNCH && sp > 0; step++) {
        ...
        // Warp efficiency sampling (also serves as implicit convergence barrier)
        mySteps++;
        if ((mySteps & 7) == 0) {
    ...
    // Write mutable state back only for active rays
    int alive = (sp > 0) ? 1 : 0;
    if (alive) {
        wf_sp[slot] = sp;
        wf_tHit[slot] = tHit;
        ...
        // Finished ray: write final result, skip stack writeback
        wf_sp[slot] = 0;
        wf_tHit[slot] = tHit;
        wf_hitTri[slot] = hitTri;
        wf_hitU[slot] = hitU; wf_hitV[slot] = hitV;
    ...
    // Fused warp-level compaction: no CUB, no separate kernel
    // Use __ballot_sync + atomicAdd for zero-overhead stream compaction
    unsigned fullMask = __activemask();
    unsigned aliveMask = __ballot_sync(fullMask, alive);
    int laneId = threadIdx.x & 31;
    int warpAlive = __popc(aliveMask);
    int warpBase = 0;
    if (laneId == 0 && warpAlive > 0)
        warpBase = atomicAdd(outCount, warpAlive);
    warpBase = __shfl_sync(fullMask, warpBase, 0);
    if (alive) {
        unsigned lowerMask = (1u << laneId) - 1;
        int myPos = __popc(aliveMask & lowerMask);
        activeMapOut[warpBase + myPos] = slot;
    }
...
int main() {
    printf("══════════════════════════════════════════════════════════════════════════\n");
    printf("  V24 — Zero-Overhead Wavefront + Constant BVH Cache + 4M Rays\n");
    ...
    for (int si = 0; si < nScenes; si++) {
        ...
        // Upload top BVH nodes to constant memory (64KB broadcast cache)
        ...
        // ──── PRIMARY INLINE BENCHMARK (Zero-HBM) ────
        {
            int side = (int)sqrtf((float)NRAYS);
            unsigned int zero = 0; cudaMemcpyToSymbol(g_rayCounter, &zero, 4);
            tracePrimaryInline<<<320, 256>>>(d_bvh4, n4, d_tv[0],d_tv[1],d_tv[2],d_tv[3],d_tv[4],d_tv[5],d_tv[6],d_tv[7],d_tv[8],
                d_hits, NRAYS, side, 0.f, 0.f, -20.f);
            cudaDeviceSynchronize();

            cudaEvent_t t0i, t1i; cudaEventCreate(&t0i); cudaEventCreate(&t1i);
            float bestI = 1e30f;
            for (int r = 0; r < 3; r++) {
                cudaMemcpyToSymbol(g_rayCounter, &zero, 4);
                cudaEventRecord(t0i);
                tracePrimaryInline<<<320, 256>>>(d_bvh4, n4, d_tv[0],d_tv[1],d_tv[2],d_tv[3],d_tv[4],d_tv[5],d_tv[6],d_tv[7],d_tv[8],
                    d_hits, NRAYS, side, 0.f, 0.f, -20.f);
                cudaEventRecord(t1i); cudaEventSynchronize(t1i);
                float ms; cudaEventElapsedTime(&ms, t0i, t1i); if (ms < bestI) bestI = ms;
            }
            cudaMemcpy(h_hits, d_hits, NRAYS * sizeof(Hit), cudaMemcpyDeviceToHost);
            int hcI = 0; for (int i = 0; i < NRAYS; i++) if (h_hits[i].tri >= 0) hcI++;
            printf("  PRIMARY (Inline):   %7.0f MR/s  hit:%5.1f%%\n", (float)NRAYS / bestI / 1000.f, 100.f * hcI / NRAYS);
            cudaEventDestroy(t0i); cudaEventDestroy(t1i);
        }

        // ──── PRIMARY INLINE + CSWAP SORTED BENCHMARK ────
        {
            int side = (int)sqrtf((float)NRAYS);
            unsigned int zero = 0; cudaMemcpyToSymbol(g_rayCounter, &zero, 4);
            tracePrimaryInlineSorted<<<320, 256>>>(d_bvh4, n4, d_tv[0],d_tv[1],d_tv[2],d_tv[3],d_tv[4],d_tv[5],d_tv[6],d_tv[7],d_tv[8],
                d_hits, NRAYS, side, 0.f, 0.f, -20.f);
            cudaDeviceSynchronize();
            cudaEvent_t t0s, t1s; cudaEventCreate(&t0s); cudaEventCreate(&t1s);
            float bestS = 1e30f;
            for (int r = 0; r < 3; r++) {
                cudaMemcpyToSymbol(g_rayCounter, &zero, 4);
                cudaEventRecord(t0s);
                tracePrimaryInlineSorted<<<320, 256>>>(d_bvh4, n4, d_tv[0],d_tv[1],d_tv[2],d_tv[3],d_tv[4],d_tv[5],d_tv[6],d_tv[7],d_tv[8],
                    d_hits, NRAYS, side, 0.f, 0.f, -20.f);
                cudaEventRecord(t1s); cudaEventSynchronize(t1s);
                float ms; cudaEventElapsedTime(&ms, t0s, t1s); if (ms < bestS) bestS = ms;
            }
            cudaMemcpy(h_hits, d_hits, NRAYS * sizeof(Hit), cudaMemcpyDeviceToHost);
            int hcS = 0; for (int i = 0; i < NRAYS; i++) if (h_hits[i].tri >= 0) hcS++;
            printf("  PRIMARY (Sort+In):  %7.0f MR/s  hit:%5.1f%%\n", (float)NRAYS / bestS / 1000.f, 100.f * hcS / NRAYS);
            cudaEventDestroy(t0s); cudaEventDestroy(t1s);
        }
        ...
        // ──── DIFFUSE WAVEFRONT BENCHMARK ────
        {
            ...
            int *d_activeMap, *d_activeMapOut;
            cudaMalloc(&d_activeMap, NRAYS * 4);
            cudaMalloc(&d_activeMapOut, NRAYS * 4);
            unsigned long long *d_we; cudaMalloc(&d_we, 16);

            // Pinned memory for zero-copy active count (no D2H cudaMemcpy needed)
            int* h_outCount;
            cudaHostAlloc(&h_outCount, 4, cudaHostAllocMapped);
            int* d_outCount;
            cudaHostGetDevicePointer(&d_outCount, h_outCount, 0);
                ...
                while (numActive > 0) {
                    *h_outCount = 0;
                    ...
                    wavefrontStep<<<blocks, 256>>>(d_bvh4, n4, d_tv[0],d_tv[1],d_tv[2],d_tv[3],d_tv[4],d_tv[5],d_tv[6],d_tv[7],d_tv[8],
                        ...
                        d_activeMap, numActive, d_activeMapOut, d_outCount, nullptr);
                    cudaDeviceSynchronize();
                    numActive = *h_outCount;
                ...
                while (numActive > 0) {
                    *h_outCount = 0;
                    ...
                    wavefrontStep<<<blocks, 256>>>(d_bvh4, n4, d_tv[0],d_tv[1],d_tv[2],d_tv[3],d_tv[4],d_tv[5],d_tv[6],d_tv[7],d_tv[8],
                        d_ray[0],d_ray[1],d_ray[2],d_ray[3],d_ray[4],d_ray[5],d_ray[6],d_ray[7],d_ray[8],
                        d_wf_sp, d_wf_stk, d_wf_tHit, d_wf_hitTri, d_wf_hitU, d_wf_hitV, d_wf_rayIdx,
                        d_activeMap, numActive, d_activeMapOut, d_outCount, d_we);
                    cudaDeviceSynchronize();
                    numActive = *h_outCount;
                    int* tmp = d_activeMap; d_activeMap = d_activeMapOut; d_activeMapOut = tmp;
                    iters++;
            ...
            cudaFree(d_activeMap); cudaFree(d_activeMapOut);
            cudaFree(d_we); cudaFreeHost(h_outCount);
            cudaEventDestroy(t0); cudaEventDestroy(t1);
        }
