/* v37 — Hybrid BVH4 / CWBVH Ray Tracer
 *
 * Best of both worlds:
 *   PRIMARY: BVH4 FP16 (64B/node) — proven 6+ GR/s from v29
 *     - Two-phase: broadphaseRoot (constant mem) → tracePrimaryDense (CSWAP sort)
 *     - 48 regs, 66% occupancy, simple decode
 *
 *   DIFFUSE: CWBVH (80B/node) — proven 625 MR/s (beats OptiX 551)
 *     - Morton-sorted rays for improved warp coherence
 *     - 8-wide nodes, uint8 quantized, Woop triangles
 *     - Dynamic fetch + triangle postponing
 *
 * Target: 6+ GR/s primary + 600+ MR/s diffuse
 */
...
int main() {
    ...
    for(int si = 0; si < 4; si++) {
        ...
        // Morton sort the diffuse rays
        computeRayMortonKeys<<<(NRAYS+255)/256,256>>>(d_rays,d_mortonKeys,d_mortonIdx,NRAYS,sceneMin,sceneInvSize);
        CK(cudaDeviceSynchronize());

        // CPU sort (simple, reliable — sort overhead not in benchmark loop)
        unsigned int* h_keys = (unsigned int*)malloc(NRAYS*sizeof(unsigned int));
        int* h_idx = (int*)malloc(NRAYS*sizeof(int));
        CK(cudaMemcpy(h_keys,d_mortonKeys,NRAYS*sizeof(unsigned int),cudaMemcpyDeviceToHost));
        CK(cudaMemcpy(h_idx,d_mortonIdx,NRAYS*sizeof(int),cudaMemcpyDeviceToHost));

        // Simple index sort by key
        std::vector<std::pair<unsigned int,int>> kv(NRAYS);
        for(int i=0;i<NRAYS;i++) kv[i]={h_keys[i],h_idx[i]};
        std::sort(kv.begin(),kv.end(),[](auto&a,auto&b){return a.first<b.first;});
        for(int i=0;i<NRAYS;i++) h_idx[i]=kv[i].second;
        CK(cudaMemcpy(d_mortonIdx,h_idx,NRAYS*sizeof(int),cudaMemcpyHostToDevice));
        free(h_keys); free(h_idx);

        // Reorder rays by Morton order
        reorderRays<<<(NRAYS+255)/256,256>>>(d_rays,d_sortedRays,d_mortonIdx,NRAYS);
        CK(cudaDeviceSynchronize());
