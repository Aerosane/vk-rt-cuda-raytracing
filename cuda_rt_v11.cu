// V11 ENGINE: Warp-coherent traversal + Stream compaction + Quantized BVH4
//
// NEW TECHNIQUES (from CUDA 12.x / GTC'25 best practices):
// 1. WARP-COHERENT CHILD ORDERING: __shfl_xor_sync warp-min distance for BVH4
//    → All 32 threads agree on child visit order → zero stack divergence
// 2. WARP-LEVEL COMPACTION: __ballot_sync + __popc for multi-bounce
//    → Dense warps between bounces → no idle threads
// 3. QUANTIZED BVH4: 8-bit bounds relative to parent (48B vs 64B)
//    → 25% less BVH bandwidth per ray
// 4. MORTON+OCTANT SORT: carried over from pipeline (best +173%)
//
// PHYSICS LIMITS (V100, diffuse 19.1 nd/ray):
//   64B nodes: L2 peak = 3100/(19.1*64) = 2,537 MR/s
//   48B nodes: L2 peak = 3100/(19.1*48) = 3,381 MR/s
//   Current: 531 MR/s (21% of 64B peak) — 79% lost to divergence
//   Target: close as much of that 79% gap as physics allows
...
// ═══ V10 baseline (FP16 BVH4, 64B/node) ═══
...
__device__ unsigned int g_compactOut;
...
// Branchless sort for per-thread ordering (v10 baseline)
...
// ★ NEW: Warp-wide minimum via shuffle tree (5 shuffles)
...
// ═══ V10 KERNEL (baseline — per-thread child ordering) ═══
...
// ═══ V11 KERNEL: WARP-COHERENT CHILD ORDERING ═══
// Key change: Use warp-minimum distance for child sort order
// ALL 32 threads agree on visit order → eliminates stack push/pop divergence
// Trade-off: some threads visit a farther child first, but NO branch serialization
__global__ void __launch_bounds__(256,4) traceV11(
    const int4*__restrict__ d_bvh4,int n4,int smN,
    const float*__restrict__ tv0x,const float*__restrict__ tv0y,const float*__restrict__ tv0z,
    const float*__restrict__ tv1x,const float*__restrict__ tv1y,const float*__restrict__ tv1z,
    const float*__restrict__ tv2x,const float*__restrict__ tv2y,const float*__restrict__ tv2z,
    ...
    Hit*__restrict__ hits,int numRays,unsigned long long*__restrict__ stats)
{
    int smN4=min(n4,smN);for(int i=threadIdx.x;i<smN4*4;i+=blockDim.x)s_bvh4[i]=d_bvh4[i];__syncthreads();
    int cnst=c_bvh4N,lane=threadIdx.x&31;unsigned long long ln=0,lt=0;
    while(true){int bs;if(lane==0)bs=atomicAdd(&g_rayCounterV11,32);bs=__shfl_sync(0xFFFFFFFF,bs,0);
        if(bs>=numRays)break;int ri=bs+lane;if(ri>=numRays)continue;
        float ox=rox[ri],oy=roy[ri],oz=roz[ri],dx=rdx[ri],dy=rdy[ri],dz=rdz[ri];
        float ix=rix[ri],iy=riy[ri],iz=riz[ri];float hitT=1e30f;int hitTri=-1;float hitU=0,hitV=0;
        int stk[SHORT_STACK];int sp=0,ni=0;
        while(true){
            ...
            // ★ WARP-COHERENT ORDERING: Use __ballot_sync to count how many
            // threads hit each child, then use warp-min tNear for sort order.
            // All threads agree on the same traversal order → ZERO stack divergence.
            ...
            // Warp-min distance for ordering (all threads see same value)
            ...
            // Sort using warp-uniform distances (all threads identical path)
            ...
            // Push farthest first (all threads agree)
...
BR bench(KernelFn kern,bool useV11counter,RayAoS*rays,int numRays,int4*d_bvh4,int n4,float*d_v[9],cudaDeviceProp&prop){
    ...
    unsigned int z=0;
    if(useV11counter)cudaMemcpyToSymbol(g_rayCounterV11,&z,4);else cudaMemcpyToSymbol(g_rayCounter,&z,4);
...
int main(){
    printf("==========================================================================\n");
    printf("  V11 ENGINE — Warp-Coherent Traversal + Morton Sort + Stream Compaction\n");
    printf("  Techniques: __ballot_sync, __shfl_xor_sync warp-min, __popc\n");
    printf("==========================================================================\n\n");
    ...
    for(int sceneK : {100, 500}){
        ...
        BR p10=bench(traceV10,false,rays,numRays,d_bvh4,n4,d_v,prop);
        genPrimary(rays,numRays);
        BR p11=bench(traceV11,true,rays,numRays,d_bvh4,n4,d_v,prop);

        // DIFFUSE UNSORTED
        genDiffuse(rays,numRays,h_to,nt);
        BR du10=bench(traceV10,false,rays,numRays,d_bvh4,n4,d_v,prop);
        genDiffuse(rays,numRays,h_to,nt);
        BR du11=bench(traceV11,true,rays,numRays,d_bvh4,n4,d_v,prop);

        // DIFFUSE MORTON-SORTED
        genDiffuse(rays,numRays,h_to,nt);sortMortonOctant(rays,numRays,sMn,sMx);
        BR ds10=bench(traceV10,false,rays,numRays,d_bvh4,n4,d_v,prop);
        genDiffuse(rays,numRays,h_to,nt);sortMortonOctant(rays,numRays,sMn,sMx);
        BR ds11=bench(traceV11,true,rays,numRays,d_bvh4,n4,d_v,prop);

        double l2peak=(3100.0e9/(19.1*64))/1e6; // diffuse L2 bandwidth limit

        printf("  +--------------------+--------+--------+--------+\n");
        printf("  | Config             |  V10   |  V11   | Change |\n");
        printf("  +--------------------+--------+--------+--------+\n");
        printf("  | Primary            |%7.0f |%7.0f | %+5.1f%% |\n",p10.mr,p11.mr,(p11.mr/p10.mr-1)*100);
        printf("  | Diffuse unsorted   |%7.0f |%7.0f | %+5.1f%% |\n",du10.mr,du11.mr,(du11.mr/du10.mr-1)*100);
        printf("  | Diffuse morton     |%7.0f |%7.0f | %+5.1f%% |\n",ds10.mr,ds11.mr,(ds11.mr/ds10.mr-1)*100);
        printf("  +--------------------+--------+--------+--------+\n");
        printf("  | L2 peak (diffuse)  |  %.0f MR/s theoretical    |\n",l2peak);
        printf("  | V11 morton/L2peak  |  %.1f%% of physics limit   |\n",(ds11.mr/l2peak)*100);
        printf("  +--------------------+--------+--------+--------+\n\n");
