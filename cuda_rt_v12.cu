// V12 ENGINE: Dual-Occupancy Adaptive Engine
//
// KEY INSIGHT FROM V11: Warp-coherent shuffles ADD overhead (20 shfl/node).
// They help only at 499K tris (large BVH, severe divergence).
// At 99K, v10 is still king for coherent rays.
//
// V12 APPROACH: Two kernels optimized for different workloads:
// 1. traceCoherent  — v10 kernel, 56 regs, (256,4)=32 warps/SM, smem+const cache
// 2. traceIncoherent — stripped kernel, 40 regs, (256,5)=40+ warps/SM, no smem cache
//    → 25% more warps → 25% more in-flight L2 requests → better latency hiding
//
// ADDITIONALLY: Tile-based ray batching for incoherent — group rays by BVH subtree
// region before tracing, so warps access similar BVH neighborhoods.
//
// PHYSICS BASIS:
//   Incoherent rays are L2 latency-bound, not bandwidth-bound.
//   With 32 uncoalesced reads/warp at ~200cy latency, need max warps/SM to hide.
//   v10: 32 warps, effective BW = 651 GB/s (21% of 3.1 TB/s)
//   v12: 40+ warps, expected BW = ~815 GB/s (26% of L2) → ~675 MR/s (+27%)
...
__device__ unsigned int g_rayCounterC;
__device__ unsigned int g_rayCounterI;
__device__ unsigned int g_rayCounterI2;
...
// ═══ KERNEL 1: COHERENT (v10 baseline — smem+const+ldg, 56 regs) ═══
extern __shared__ int4 s_bvh4[];
__global__ void __launch_bounds__(256,4) traceCoherent(
    ...
    Hit*__restrict__ hits,int numRays,unsigned long long*__restrict__ stats)
{
    ...
    while(true){int bs;if(lane==0)bs=atomicAdd(&g_rayCounterC,32);bs=__shfl_sync(0xFFFFFFFF,bs,0);
...
// ═══ KERNEL 2: INCOHERENT (stripped for max occupancy) ═══
// Differences from Coherent:
// - NO shared memory BVH cache (saves 2KB smem → more blocks/SM)  
// - NO constant memory BVH (saves const-cache pressure)
// - __ldg only (tex cache still helps — L1 tex is per-SM, independent)
// - Stack depth 8 (saves 4 int regs × 2 = 16 bytes)
// - Simpler child selection: test 2 children that ANY thread in warp needs
//   using __ballot_sync — but DON'T sort by distance (avoids shuffle overhead)
// - launch_bounds(256,5) → target 40 warps/SM = 25% more occupancy
__global__ void __launch_bounds__(256,5) traceIncoherent(
    ...
    Hit*__restrict__ hits,int numRays,unsigned long long*__restrict__ stats)
{
    // No smem loading — go straight to work
    int lane=threadIdx.x&31;unsigned long long ln=0,lt=0;
    while(true){int bs;if(lane==0)bs=atomicAdd(&g_rayCounterI,32);bs=__shfl_sync(0xFFFFFFFF,bs,0);
        if(bs>=numRays)break;int ri=bs+lane;if(ri>=numRays)continue;
        float ox=rox[ri],oy=roy[ri],oz=roz[ri],dx=rdx[ri],dy=rdy[ri],dz=rdz[ri];
        float ix=rix[ri],iy=riy[ri],iz=riz[ri];float hitT=1e30f;int hitTri=-1;float hitU=0,hitV=0;
        ...
        while(true){
            ...
            if(ni<=-2){int val=-(ni+2),tc=(val&7)+1,ts=val>>3;
                for(int i=0;i<tc;i++){int ti=ts+i;lt++;
                    ...
                    if(tt>0.001f&&tt<hitT){hitT=tt;hitTri=ti;hitU=u;hitV=v;}}
                if(sp>0)ni=stk[--sp];else break;continue;}
            // __ldg ONLY — no smem/const indirection branches
...
// ═══ KERNEL 3: INCOHERENT v2 — maxrregcount=48 forced ═══
// Same as traceIncoherent but compiler forced to 48 regs max
// Spills go to local memory (L1 cached on Volta), but more warps fit
#pragma nv_diag_suppress 1886
__global__ void __launch_bounds__(256,6) traceIncoherentHiOcc(
    const int4*__restrict__ d_bvh4,int n4,int _unused,
    const float*__restrict__ tv0x,const float*__restrict__ tv0y,const float*__restrict__ tv0z,
    const float*__restrict__ tv1x,const float*__restrict__ tv1y,const float*__restrict__ tv1z,
    const float*__restrict__ tv2x,const float*__restrict__ tv2y,const float*__restrict__ tv2z,
...
{
    int lane=threadIdx.x&31;unsigned long long ln=0,lt=0;
    while(true){int bs;if(lane==0)bs=atomicAdd(&g_rayCounterI2,32);bs=__shfl_sync(0xFFFFFFFF,bs,0);
        if(bs>=numRays)break;int ri=bs+lane;if(ri>=numRays)continue;
        float ox=rox[ri],oy=roy[ri],oz=roz[ri],dx=rdx[ri],dy=rdy[ri],dz=rdz[ri];
        float ix=rix[ri],iy=riy[ri],iz=riz[ri];float hitT=1e30f;int hitTri=-1;float hitU=0,hitV=0;
        // Minimal stack — only 6 entries to save registers
        int stk[6];int sp=0,ni=0;
        while(true){
            if(ni==-1){if(sp>0)ni=stk[--sp];else break;continue;}
            if(ni<=-2){int val=-(ni+2),tc=(val&7)+1,ts=val>>3;
                for(int i=0;i<tc;i++){int ti=ts+i;lt++;
            ...
            bool h3=(c3!=-1)&&(t3x>=fmaxf(t3n,0.f))&&(t3n<hitT);
            int nh=h0+h1+h2+h3;if(nh==0){if(sp>0)ni=stk[--sp];else break;continue;}
            // Simplified: just push first 2 hits, visit nearest
            float dd[4]={h0?t0n:1e30f,h1?t1n:1e30f,h2?t2n:1e30f,h3?t3n:1e30f};
            int ci[4]={c0,c1,c2,c3};
            cswap(dd[0],dd[1],ci[0],ci[1]);cswap(dd[2],dd[3],ci[2],ci[3]);
            cswap(dd[0],dd[2],ci[0],ci[2]);cswap(dd[1],dd[3],ci[1],ci[3]);cswap(dd[1],dd[2],ci[1],ci[2]);
            for(int i=nh-1;i>=1&&sp<6;i--)stk[sp++]=ci[i];ni=ci[0];continue;}
        hits[ri].t=hitT;hits[ri].tri=hitTri;hits[ri].u=hitU;hits[ri].v=hitV;}
    atomicAdd(&stats[0],ln);atomicAdd(&stats[1],lt);}

// ═══ SCENE + SORTING (identical to v10/v11) ═══
...
BR bench(KernelFn kern,unsigned int*d_counter_sym,RayAoS*rays,int numRays,int4*d_bvh4,int n4,
         float*d_v[9],cudaDeviceProp&prop,bool useSmem){
    ...
    int smB=useSmem?SMEM_BVH4_NODES*4*(int)sizeof(int4):0;
    unsigned int z=0;cudaMemcpyToSymbol(*d_counter_sym?g_rayCounterC:g_rayCounterC,&z,4); // reset below
    // Warmup
    cudaMemcpyToSymbol(g_rayCounterC,&z,4);cudaMemcpyToSymbol(g_rayCounterI,&z,4);cudaMemcpyToSymbol(g_rayCounterI2,&z,4);
    kern<<<nb,256,smB>>>(d_bvh4,n4,SMEM_BVH4_NODES,d_v[0],d_v[1],d_v[2],d_v[3],d_v[4],d_v[5],d_v[6],d_v[7],d_v[8],
...
int main(){
    printf("══════════════════════════════════════════════════════════════════════════\n");
    printf("  V12 ENGINE — Dual-Occupancy Adaptive Traversal\n");
    printf("  Coherent: smem+const+ldg, 56 reg, (256,4)=32 warps/SM\n");
    printf("  Incoherent: ldg-only, stripped, (256,5)=40 warps/SM\n");
    printf("  IncoherentHiOcc: ultra-stripped, (256,6)=48 warps/SM\n");
    printf("══════════════════════════════════════════════════════════════════════════\n\n");
    ...
    for(int sceneK : {100, 500}){
        ...
        // Test 1: Primary rays (coherent)
        genPrimary(rays,numRays);
        BR pc=bench(traceCoherent,(unsigned int*)0,rays,numRays,d_bvh4,n4,d_v,prop,true);
        BR pi=bench(traceIncoherent,(unsigned int*)0,rays,numRays,d_bvh4,n4,d_v,prop,false);
        BR pi2=bench(traceIncoherentHiOcc,(unsigned int*)0,rays,numRays,d_bvh4,n4,d_v,prop,false);
        printf("  +-----------------------------------+--------+--------+--------+\n");
        printf("  | PRIMARY RAYS                      | Coher  | Incohr | HiOcc  |\n");
        printf("  +-----------------------------------+--------+--------+--------+\n");
        printf("  | MRays/s                           | %6.0f | %6.0f | %6.0f |\n",pc.mr,pi.mr,pi2.mr);
        printf("  | Nodes/ray                         | %6.1f | %6.1f | %6.1f |\n",pc.ndR,pi.ndR,pi2.ndR);
        printf("  | Bytes/ray                         | %6.0f | %6.0f | %6.0f |\n",pc.bpr,pi.bpr,pi2.bpr);
        printf("  | Hit%%                              | %5d%% | %5d%% | %5d%% |\n",pc.hitPct,pi.hitPct,pi2.hitPct);
        printf("  +-----------------------------------+--------+--------+--------+\n\n");

        // Test 2: Diffuse unsorted
        genDiffuse(rays,numRays,h_to,nt);
        BR dc=bench(traceCoherent,(unsigned int*)0,rays,numRays,d_bvh4,n4,d_v,prop,true);
        BR di=bench(traceIncoherent,(unsigned int*)0,rays,numRays,d_bvh4,n4,d_v,prop,false);
        BR di2=bench(traceIncoherentHiOcc,(unsigned int*)0,rays,numRays,d_bvh4,n4,d_v,prop,false);
        printf("  +-----------------------------------+--------+--------+--------+\n");
        printf("  | DIFFUSE UNSORTED                  | Coher  | Incohr | HiOcc  |\n");
        printf("  +-----------------------------------+--------+--------+--------+\n");
        printf("  | MRays/s                           | %6.0f | %6.0f | %6.0f |\n",dc.mr,di.mr,di2.mr);
        printf("  | Nodes/ray                         | %6.1f | %6.1f | %6.1f |\n",dc.ndR,di.ndR,di2.ndR);
        printf("  +-----------------------------------+--------+--------+--------+\n\n");

        // Test 3: Diffuse Morton+Octant sorted
        sortMortonOctant(rays,numRays,sMn,sMx);
        BR dmc=bench(traceCoherent,(unsigned int*)0,rays,numRays,d_bvh4,n4,d_v,prop,true);
        BR dmi=bench(traceIncoherent,(unsigned int*)0,rays,numRays,d_bvh4,n4,d_v,prop,false);
        BR dmi2=bench(traceIncoherentHiOcc,(unsigned int*)0,rays,numRays,d_bvh4,n4,d_v,prop,false);
        printf("  +-----------------------------------+--------+--------+--------+\n");
        printf("  | DIFFUSE MORTON+OCTANT             | Coher  | Incohr | HiOcc  |\n");
        printf("  +-----------------------------------+--------+--------+--------+\n");
        printf("  | MRays/s                           | %6.0f | %6.0f | %6.0f |\n",dmc.mr,dmi.mr,dmi2.mr);
        printf("  | Nodes/ray                         | %6.1f | %6.1f | %6.1f |\n",dmc.ndR,dmi.ndR,dmi2.ndR);
        printf("  +-----------------------------------+--------+--------+--------+\n");

        // Summary
        printf("\n  ★ BEST for Primary:  %.0f MR/s (%s)\n",fmax(pc.mr,fmax(pi.mr,pi2.mr)),
            pc.mr>=pi.mr&&pc.mr>=pi2.mr?"Coherent":pi.mr>=pi2.mr?"Incoherent":"HiOcc");
        printf("  ★ BEST for Diffuse:  %.0f MR/s (%s, Morton)\n",fmax(dmc.mr,fmax(dmi.mr,dmi2.mr)),
            dmc.mr>=dmi.mr&&dmc.mr>=dmi2.mr?"Coherent":dmi.mr>=dmi2.mr?"Incoherent":"HiOcc");
        double l2peak=3100.0e9/(19.1*64)/1e6;
        printf("  ★ Diffuse/L2 peak:   %.1f%% (%.0f MR/s theoretical)\n\n",
            fmax(dmc.mr,fmax(dmi.mr,dmi2.mr))/l2peak*100,l2peak);

        free(rays);free(h_tris);free(h_nodes);free(tidx);free(h_ord);free(remap);free(h_to);
        free(h_b4);free(h_b4o);free(r4);free(h_gpu);
        for(int j=0;j<9;j++){free(h_v[j]);cudaFree(d_v[j]);}cudaFree(d_bvh4);
    }
    return 0;
}
