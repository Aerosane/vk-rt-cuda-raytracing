// V13 ENGINE: Two-Level Dispatch + Full L1 Exploitation
//
// WHITEPAPER INSIGHTS APPLIED:
// 1. UNIFIED L1/SMEM (128KB): No shared memory → all 128KB as L1 data cache
//    → traceIncoherent (v12) proved this: +6.6% diffuse, +4% primary over smem version
// 2. UNLIMITED CACHE MISSES IN FLIGHT: Volta L1 is non-blocking
//    → High occupancy (40 warps/SM) keeps L2 pipeline full even with divergent access
// 3. SIMULTANEOUS FP32+INT32: Stack push/pop, address calc on INT32 cores
//    → Free computation alongside FP32 AABB slab tests
// 4. PER-THREAD PC: Convergence optimizer regroups diverged threads
//    → Better than Pascal's rigid reconvergence for BVH traversal
//
// NEW IN V13: TWO-LEVEL DISPATCH
// Problem: At 499K tris, BVH = 4.33 MB >> 128KB L1. Every node access misses L1.
// Solution: Sort rays by BVH subtree → each warp's working set fits in L1.
//   Step 1: Shallow traversal (top 3 levels, ~21 nodes, fits in registers)
//           → assigns each ray to a level-3 subtree
//   Step 2: Sort rays by subtree ID
//   Step 3: Trace sorted rays → warp accesses one subtree → fits in L1!
//
// At 99K: 14504 nodes / 64 subtrees ≈ 227 nodes/subtree = 14.5 KB → fits in 128KB L1
// At 499K: 70980 nodes / 64 subtrees ≈ 1109 nodes/subtree = 69 KB → fits in 128KB L1!
// vs unsorted: 4.33 MB working set → L1 thrashes constantly
...
// ═══ STEP 1: SHALLOW BVH CLASSIFIER ═══
// Traces each ray through top 3 levels of BVH4 (root + 4 children + up to 16 grandchildren)
// Assigns each ray a subtree ID = the deepest internal node it would enter
// This is done ONCE per ray, very cheaply (~21 node tests, all L1-cached)
__global__ void __launch_bounds__(256,5) classifyRays(
    ...
    int*__restrict__ subtreeIds,int numRays,int maxDepth)
{
    int idx=blockIdx.x*blockDim.x+threadIdx.x;
    if(idx>=numRays)return;
    float ox=rox[idx],oy=roy[idx],oz=roz[idx],dx=rdx[idx],dy=rdy[idx],dz=rdz[idx];
    float ix=rix[idx],iy=riy[idx],iz=riz[idx];
    // Traverse top levels of BVH4 — find which subtree this ray belongs to
    int ni=0;
    for(int depth=0;depth<maxDepth;depth++){
        if(ni<0)break; // hit a leaf, done
        ...
        bool h0=(c0!=-1&&c0>=0)&&(t0x2>=fmaxf(t0n,0.f));
        bool h1=(c1!=-1&&c1>=0)&&(t1x2>=fmaxf(t1n,0.f));
        bool h2=(c2!=-1&&c2>=0)&&(t2x2>=fmaxf(t2n,0.f));
        bool h3=(c3!=-1&&c3>=0)&&(t3x2>=fmaxf(t3n,0.f));
        // Pick nearest hit internal child to descend into
        float best=1e30f;int bestC=-1;
        if(h0&&t0n<best){best=t0n;bestC=c0;}
        if(h1&&t1n<best){best=t1n;bestC=c1;}
        if(h2&&t2n<best){best=t2n;bestC=c2;}
        if(h3&&t3n<best){best=t3n;bestC=c3;}
        if(bestC<0)break;
        ni=bestC;
    }
    // ni is now the subtree root node index for this ray
    // Use max(ni,0) as the subtree ID
    subtreeIds[idx]=ni>=0?ni:0;
}

// ═══ MAIN TRACE KERNEL: v12 Incoherent (proven best) ═══
// Pure __ldg, 128KB L1, 48 regs, (256,5)=40 warps/SM
__global__ void __launch_bounds__(256,5) traceV13(
    const int4*__restrict__ d_bvh4,int n4,int _unused,
...
{
    int lane=threadIdx.x&31;unsigned long long ln=0,lt=0;
    while(true){int bs;if(lane==0)bs=atomicAdd(&g_rayCounter,32);bs=__shfl_sync(0xFFFFFFFF,bs,0);
        ...
        int stk[INCOHERENT_STACK];int sp=0,ni=0;
        while(true){
            ...
            for(int i=nh-1;i>=1&&sp<INCOHERENT_STACK;i--)stk[sp++]=ci[i];ni=ci[0];continue;}
...
// ═══ TWO-LEVEL DISPATCH: Subtree sort ═══
// Sort rays by their classified subtree ID, then by Morton within subtree
void sortBySubtree(RayAoS*r,int*subtreeIds,int n,float3 smn,float3 smx){
    struct SK{uint64_t key;int idx;};SK*keys=(SK*)malloc(n*sizeof(SK));
    for(int i=0;i<n;i++){
        uint32_t m=morton3D(r[i].o.x,r[i].o.y,r[i].o.z,smn,smx);
        uint32_t o=(uint32_t)octant(r[i].d);
        // Key: [subtreeId:20][octant:3][morton:9] — subtree is primary sort
        keys[i]={((uint64_t)subtreeIds[i]<<12)|((uint64_t)o<<9)|(m>>21),i};
    }
    std::sort(keys,keys+n,[](const SK&a,const SK&b){return a.key<b.key;});
    RayAoS*tmp=(RayAoS*)malloc(n*sizeof(RayAoS));
    int*tmpIds=(int*)malloc(n*4);
    for(int i=0;i<n;i++){tmp[i]=r[keys[i].idx];tmpIds[i]=subtreeIds[keys[i].idx];}
    memcpy(r,tmp,n*sizeof(RayAoS));memcpy(subtreeIds,tmpIds,n*4);
    free(tmp);free(tmpIds);free(keys);}
...
struct BR{double mr,ndR,trR,bpr;int hitPct;};
BR bench(KernelFn kern,RayAoS*rays,int numRays,int4*d_bvh4,int n4,float*d_v[9],cudaDeviceProp&prop){
    ...
    kern<<<nb,256>>>(d_bvh4,n4,0,d_v[0],d_v[1],d_v[2],d_v[3],d_v[4],d_v[5],d_v[6],d_v[7],d_v[8],
    ...
    for(int r=0;r<10;r++){
        cudaMemcpyToSymbol(g_rayCounter,&z,4);cudaMemset(d_st,0,16);cudaEventRecord(t0);
        kern<<<nb,256>>>(d_bvh4,n4,0,d_v[0],d_v[1],d_v[2],d_v[3],d_v[4],d_v[5],d_v[6],d_v[7],d_v[8],
            d_ray[0],d_ray[1],d_ray[2],d_ray[3],d_ray[4],d_ray[5],d_ray[6],d_ray[7],d_ray[8],d_hits,numRays,d_st);
    ...
    BR res;res.mr=(double)numRays/((total/10)/1000.0)/1e6;
    res.ndR=(double)st[0]/numRays;res.trR=(double)st[1]/numRays;res.bpr=res.ndR*64+res.trR*36;res.hitPct=100*hc/numRays;
    cudaEventDestroy(t0);cudaEventDestroy(t1);
    for(int j=0;j<9;j++){cudaFree(d_ray[j]);free(h_ray[j]);}cudaFree(d_hits);cudaFree(d_st);free(hh);return res;}

int main(){
    printf("══════════════════════════════════════════════════════════════════════════\n");
    printf("  V13 ENGINE — Two-Level Dispatch + Full 128KB L1 Exploitation\n");
    printf("  Whitepaper insights: unified L1/smem, unlimited misses, FP32+INT32\n");
    printf("══════════════════════════════════════════════════════════════════════════\n\n");

    // NOTE: cudaFuncCachePreferL1 is no-op on compute 7.x+ (Volta manages L1/smem dynamically)
    // Removed: was causing anomalous 3x diffuse regression on driver 580.126.20
    ...
    for(int sceneK : {100, 500}){
        ...
        cudaMemcpyToSymbol(c_bvh4,h_gpu,cN*4*sizeof(int4));cudaMemcpyToSymbol(c_bvh4N,&cN,4);
        ...
        printf("  === %dK tris | %d BVH4 nodes | %.2f MB BVH ===\n",nt/1000,n4,n4*64.0/1048576);
        printf("  L1 per-subtree (depth-2): ~%.1f KB (L1=128KB: %s)\n",
            n4*64.0/1024.0/16, n4*64.0/16<128*1024?"FITS!":"overflow");

        RayAoS*rays=(RayAoS*)malloc(numRays*sizeof(RayAoS));

        // ═══ TEST 1: Primary (already coherent — baseline) ═══
        genPrimary(rays,numRays);
        BR p=bench(traceV13,rays,numRays,d_bvh4,n4,d_v,prop);
        printf("\n  PRIMARY RAYS:           %6.0f MR/s | %4.1f nd/r | %3.0f B/r | hit %d%%\n",p.mr,p.ndR,p.bpr,p.hitPct);

        // ═══ TEST 2: Diffuse unsorted (worst case) ═══
        genDiffuse(rays,numRays,h_to,nt);
        BR du=bench(traceV13,rays,numRays,d_bvh4,n4,d_v,prop);
        printf("  DIFFUSE UNSORTED:       %6.0f MR/s | %4.1f nd/r\n",du.mr,du.ndR);

        // ═══ TEST 3: Morton+Octant sort (v10 best practice) ═══
        genDiffuse(rays,numRays,h_to,nt);
        sortMortonOctant(rays,numRays,sMn,sMx);
        BR dm=bench(traceV13,rays,numRays,d_bvh4,n4,d_v,prop);
        printf("  DIFFUSE MORTON+OCTANT:  %6.0f MR/s | %4.1f nd/r\n",dm.mr,dm.ndR);

        // ═══ TEST 4: TWO-LEVEL DISPATCH (NEW!) ═══
        genDiffuse(rays,numRays,h_to,nt);
        // Upload rays for classification
        float*h_ray[9];for(int j=0;j<9;j++)h_ray[j]=(float*)malloc(numRays*4);
        for(int i=0;i<numRays;i++){h_ray[0][i]=rays[i].o.x;h_ray[1][i]=rays[i].o.y;h_ray[2][i]=rays[i].o.z;
            h_ray[3][i]=rays[i].d.x;h_ray[4][i]=rays[i].d.y;h_ray[5][i]=rays[i].d.z;
            h_ray[6][i]=rays[i].id.x;h_ray[7][i]=rays[i].id.y;h_ray[8][i]=rays[i].id.z;}
        float*d_ray[9];
        for(int j=0;j<9;j++){cudaMalloc(&d_ray[j],numRays*4);cudaMemcpy(d_ray[j],h_ray[j],numRays*4,cudaMemcpyHostToDevice);}
        int*d_subtreeIds;cudaMalloc(&d_subtreeIds,numRays*4);
        // Classify rays through top 2 levels of BVH4 (root→children→grandchildren)
        int classDepth=2;
        classifyRays<<<(numRays+255)/256,256>>>(d_bvh4,n4,d_ray[0],d_ray[1],d_ray[2],
            d_ray[3],d_ray[4],d_ray[5],d_ray[6],d_ray[7],d_ray[8],d_subtreeIds,numRays,classDepth);
        cudaDeviceSynchronize();
        int*h_subtreeIds=(int*)malloc(numRays*4);
        cudaMemcpy(h_subtreeIds,d_subtreeIds,numRays*4,cudaMemcpyDeviceToHost);
        // Count unique subtrees
        int maxSub=0;for(int i=0;i<numRays;i++)if(h_subtreeIds[i]>maxSub)maxSub=h_subtreeIds[i];
        int*subCounts=(int*)calloc(maxSub+1,4);
        for(int i=0;i<numRays;i++)subCounts[h_subtreeIds[i]]++;
        int uniqueSubs=0;for(int i=0;i<=maxSub;i++)if(subCounts[i]>0)uniqueSubs++;
        printf("  [classify] depth=%d → %d unique subtrees (max node %d)\n",classDepth,uniqueSubs,maxSub);
        // Sort rays by subtree, then Morton within subtree
        sortBySubtree(rays,h_subtreeIds,numRays,sMn,sMx);
        BR ds=bench(traceV13,rays,numRays,d_bvh4,n4,d_v,prop);
        printf("  DIFFUSE SUBTREE-SORT:   %6.0f MR/s | %4.1f nd/r\n",ds.mr,ds.ndR);

        // ═══ TEST 5: Subtree sort + Morton within subtree (combined) ═══
        // Already done in sortBySubtree (it includes Morton)
        // But let's also try a depth-3 classification
        genDiffuse(rays,numRays,h_to,nt);
        for(int j=0;j<9;j++){
            for(int i=0;i<numRays;i++){
                if(j==0)h_ray[j][i]=rays[i].o.x;else if(j==1)h_ray[j][i]=rays[i].o.y;else if(j==2)h_ray[j][i]=rays[i].o.z;
                else if(j==3)h_ray[j][i]=rays[i].d.x;else if(j==4)h_ray[j][i]=rays[i].d.y;else if(j==5)h_ray[j][i]=rays[i].d.z;
                else if(j==6)h_ray[j][i]=rays[i].id.x;else if(j==7)h_ray[j][i]=rays[i].id.y;else h_ray[j][i]=rays[i].id.z;
            }
            cudaMemcpy(d_ray[j],h_ray[j],numRays*4,cudaMemcpyHostToDevice);
        }
        classDepth=3;
        classifyRays<<<(numRays+255)/256,256>>>(d_bvh4,n4,d_ray[0],d_ray[1],d_ray[2],
            d_ray[3],d_ray[4],d_ray[5],d_ray[6],d_ray[7],d_ray[8],d_subtreeIds,numRays,classDepth);
        cudaDeviceSynchronize();
        cudaMemcpy(h_subtreeIds,d_subtreeIds,numRays*4,cudaMemcpyDeviceToHost);
        maxSub=0;for(int i=0;i<numRays;i++)if(h_subtreeIds[i]>maxSub)maxSub=h_subtreeIds[i];
        free(subCounts);subCounts=(int*)calloc(maxSub+1,4);
        for(int i=0;i<numRays;i++)subCounts[h_subtreeIds[i]]++;
        uniqueSubs=0;for(int i=0;i<=maxSub;i++)if(subCounts[i]>0)uniqueSubs++;
        printf("  [classify] depth=%d → %d unique subtrees (max node %d)\n",classDepth,uniqueSubs,maxSub);
        float avgNodesPerSub=n4/(float)uniqueSubs;
        printf("  [estimate] ~%.0f nodes/subtree = ~%.1f KB → %s\n",
            avgNodesPerSub,avgNodesPerSub*64/1024,avgNodesPerSub*64<128*1024?"fits L1":"exceeds L1");
        sortBySubtree(rays,h_subtreeIds,numRays,sMn,sMx);
        BR ds3=bench(traceV13,rays,numRays,d_bvh4,n4,d_v,prop);
        printf("  DIFFUSE SUBTREE-D3:     %6.0f MR/s | %4.1f nd/r\n",ds3.mr,ds3.ndR);

        // Summary
        double best=fmax(fmax(fmax(dm.mr,ds.mr),ds3.mr),du.mr);
        double l2peak=3100.0e9/(19.1*64)/1e6;
        printf("\n  ★ BEST Primary:     %6.0f MR/s\n",p.mr);
        printf("  ★ BEST Diffuse:     %6.0f MR/s (%s)\n",best,
            best==dm.mr?"Morton":best==ds.mr?"Subtree-D2":best==ds3.mr?"Subtree-D3":"Unsorted");
        printf("  ★ vs v10 Morton:    %+.1f%%\n",(best-542.0)/542.0*100);
        printf("  ★ vs v12 Incohr:    %+.1f%%\n",(best-563.0)/563.0*100);
        printf("  ★ L2 peak %%:        %.1f%% of %.0f MR/s\n",best/l2peak*100,l2peak);
        printf("  ══════════════════════════════════════════\n\n");

        free(rays);free(h_subtreeIds);free(subCounts);free(h_tris);free(h_nodes);free(tidx);
        free(h_ord);free(remap);free(h_to);free(h_b4);free(h_b4o);free(r4);free(h_gpu);
        for(int j=0;j<9;j++){free(h_v[j]);free(h_ray[j]);cudaFree(d_v[j]);cudaFree(d_ray[j]);}
        cudaFree(d_bvh4);cudaFree(d_subtreeIds);
    }
    return 0;
}
