/* v31 — Production PBR Path Tracer (OptiX-class feature set)
 *
 * Built on v29's proven BVH4 FP16 engine (6263 MR/s).
 * Focus: SPEED + PRECISION over denoising.
 *
 * Features:
 *   ● Cook-Torrance GGX specular (roughness + metallic)
 *   ● Dielectric glass (Snell refraction, Fresnel, Beer absorption)
 *   ● Multiple Importance Sampling (power heuristic, BRDF + NEE)
 *   ● Cosine-weighted diffuse IS
 *   ● GGX VNDF importance sampling (specular)
 *   ● Thin-lens depth of field
 *   ● Analytic sphere primitives (mirror, glass)
 *   ● Procedural environment (gradient sky)
 *   ● Russian roulette, firefly clamp
 *   ● Adaptive sampling (variance-guided ray budget)
 *   ● ACES filmic tonemapping
 *   ● G-buffer (depth, normals, albedo)
 *
 * Build: nvcc -O3 -arch=sm_70 --use_fast_math -o v31 cuda_rt_v31.cu
 * Run:   ./v31 [spp] [width]   (defaults: 256 spp, 512 width)
 */
...
// Material types
enum MatType : uint8_t { MAT_DIFFUSE=0, MAT_METAL=1, MAT_GLASS=2, MAT_EMIT=3 };
...
__global__ void denoise(
    ...
    const GBuf* __restrict__ gbuf, int W, int H, int spp,
    float sigS, float sigD, float sigN)
{
    ...
    for (int dy=-R; dy<=R; dy++) for (int dx=-R; dx<=R; dx++) {
        ...
        sumR += inR[nidx]*invSpp*w; sumG += inG[nidx]*invSpp*w; sumB += inB[nidx]*invSpp*w; sumW += w;
    }
...
__global__ void tonemap(
    const float* __restrict__ inR, const float* __restrict__ inG, const float* __restrict__ inB,
    uint8_t* __restrict__ out, int W, int H, float exposure, bool divSpp, int spp)
{
    int px = blockIdx.x * blockDim.x + threadIdx.x;
    int py = blockIdx.y * blockDim.y + threadIdx.y;
    if (px >= W || py >= H) return;
    int pidx = py * W + px;
    float s = exposure * (divSpp ? (1.f/spp) : 1.f);
...
// ===================== Cornell Box + Spheres Scene =====================
static void genCornellBox(std::vector<Tri>& tris, std::vector<int>& matIds) {
    ...
    addQuad({bx0,by1,bz0},{bx1,by1,bz0},{bx1,by1,bz1},{bx0,by1,bz1}, 0);
    addQuad({bx0,0,bz0},{bx0,0,bz1},{bx0,by1,bz1},{bx0,by1,bz0}, 0);
    addQuad({bx1,0,bz0},{bx1,by1,bz0},{bx1,by1,bz1},{bx1,0,bz1}, 0);
    addQuad({bx0,0,bz0},{bx1,0,bz0},{bx1,by1,bz0},{bx0,by1,bz0}, 0);
    addQuad({bx0,0,bz1},{bx0,by1,bz1},{bx1,by1,bz1},{bx1,0,bz1}, 0);
    // Tall box
    float tx0=2.85f,tx1=4.48f,ty1=3.30f,tz0=3.00f,tz1=4.60f;
    addQuad({tx0,ty1,tz0},{tx1,ty1,tz0},{tx1,ty1,tz1},{tx0,ty1,tz1}, 0);
    addQuad({tx0,0,tz0},{tx0,0,tz1},{tx0,ty1,tz1},{tx0,ty1,tz0}, 0);
    addQuad({tx1,0,tz0},{tx1,ty1,tz0},{tx1,ty1,tz1},{tx1,0,tz1}, 0);
    addQuad({tx0,0,tz0},{tx1,0,tz0},{tx1,ty1,tz0},{tx0,ty1,tz0}, 0);
    addQuad({tx0,0,tz1},{tx0,ty1,tz1},{tx1,ty1,tz1},{tx1,0,tz1}, 0);
}

// ===================== Main =====================
int main(int argc, char** argv) {
    int SPP = (argc > 1) ? atoi(argv[1]) : 256;
    int W = (argc > 2) ? atoi(argv[2]) : 512;
    int H = W;

    printf("══════════════════════════════════════════════════════════════════════════\n");
    printf("  V31 — PBR Path Tracer (OptiX-class)\n");
    printf("  %dx%d | %d spp | %d bounces\n", W, H, SPP, MAX_BOUNCES);
    printf("  GGX specular, glass refraction, MIS, DOF, ACES tonemap\n");
    printf("══════════════════════════════════════════════════════════════════════════\n\n");
    ...
    // ---- Materials ----
    // 0: white diffuse, 1: red diffuse, 2: green diffuse, 3: light emitter
    // 4: mirror metal, 5: glass
    Material h_mat[6] = {
        {0.73f,0.73f,0.73f, 0,0,0, 1.0f, 0.f, 1.5f, MAT_DIFFUSE},  // white
        {0.65f,0.05f,0.05f, 0,0,0, 1.0f, 0.f, 1.5f, MAT_DIFFUSE},  // red
        {0.12f,0.45f,0.15f, 0,0,0, 1.0f, 0.f, 1.5f, MAT_DIFFUSE},  // green
        {0.0f, 0.0f, 0.0f,  17.f,12.f,4.f, 1.0f, 0.f, 1.5f, MAT_EMIT}, // light
        {0.95f,0.93f,0.88f, 0,0,0, 0.02f, 1.f, 1.5f, MAT_METAL},   // mirror
        {0.05f,0.05f,0.05f, 0,0,0, 0.0f, 0.f, 1.5f, MAT_GLASS},    // glass (low absorption = clear)
    };
    int nMat = 6;
    ...
    for (int s = 0; s < SPP; s++) {
        pathTrace<<<grid,block>>>(d_bvh4, n4,
            ...
            d_matIds, d_aR,d_aG,d_aB, d_gbuf, W,H, s,
    ...
    long long totalRays = (long long)nPx * SPP; // primary rays, bounces are extra
    printf("  Trace: %.1f ms (%.1f ms/spp)\n", traceMs, traceMs/SPP);
    printf("  Primary throughput: %.2f MR/s\n", (float)totalRays/traceMs/1000.f);

    // ---- Save raw ----
    uint8_t* d_rgb; cudaMalloc(&d_rgb, nPx*3);
    uint8_t* h_rgb = (uint8_t*)malloc(nPx*3);

    tonemap<<<grid,block>>>(d_aR,d_aG,d_aB, d_rgb, W,H, 1.0f, true, SPP);
    cudaMemcpy(h_rgb, d_rgb, nPx*3, cudaMemcpyDeviceToHost);
    char fn[256];
    snprintf(fn,sizeof(fn),"/workspaces/codespace/VK_RT/v31_raw_%dspp.ppm",SPP);
    ...
    cudaEventRecord(td0);
    denoise<<<grid,block>>>(d_aR,d_aG,d_aB, d_dR,d_dG,d_dB, d_gbuf, W,H,SPP, 3.f,0.1f,0.1f);
    cudaEventRecord(td1); cudaEventSynchronize(td1);
    float denMs; cudaEventElapsedTime(&denMs, td0, td1);
    printf("  Denoise: %.2f ms\n", denMs);

    tonemap<<<grid,block>>>(d_dR,d_dG,d_dB, d_rgb, W,H, 1.0f, false, 1);
    cudaMemcpy(h_rgb, d_rgb, nPx*3, cudaMemcpyDeviceToHost);
    snprintf(fn,sizeof(fn),"/workspaces/codespace/VK_RT/v31_denoised_%dspp.ppm",SPP);
    writePPM(fn, h_rgb, W, H);
    ...
    for(int m=0;m<3;m++){
        ...
        cudaMemcpy(h_rgb,d_rgb,nPx*3,cudaMemcpyDeviceToHost);
        snprintf(fn,sizeof(fn),"/workspaces/codespace/VK_RT/v31_gbuf_%s.ppm",gn[m]);
        writePPM(fn,h_rgb,W,H);
    }
    ...
    printf("  ✅ Thin-lens depth of field (aperture=%.2f)\n", aperture);
    ...
    printf("  ✅ G-buffer (depth, normals, albedo)\n");

    printf("\n  ─── Timings ───\n");
    printf("  Path trace: %.1f ms (%d spp × %d px)\n", traceMs, SPP, nPx);
    printf("  Denoise:    %.2f ms\n", denMs);
    printf("  Total:      %.1f ms\n", traceMs + denMs);
    ...
    cudaFree(d_bvh4); cudaFree(d_matIds); cudaFree(d_aR); cudaFree(d_aG); cudaFree(d_aB);
