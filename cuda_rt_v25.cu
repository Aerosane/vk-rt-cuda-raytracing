/* v24 — Zero-Overhead Wavefront BVH4 Engine
 *
...
// ======================== PRIMARY HiOcc: maxrregcount=32 → 100% occupancy ========================
// Force 32 regs via maxrregcount pragma. Spills go to LMEM (L1 cached).
// 2048 threads/SM hide L2 latency better than 1280 threads at 45 regs.
#pragma push
#pragma nv_maxrregcount(40)
__global__ void __launch_bounds__(256, 6) tracePrimaryHiOcc(
    const int4* __restrict__ bvh, int n4,
    const float* __restrict__ tv0x, const float* __restrict__ tv0y, const float* __restrict__ tv0z,
    const float* __restrict__ tv1x, const float* __restrict__ tv1y, const float* __restrict__ tv1z,
    const float* __restrict__ tv2x, const float* __restrict__ tv2y, const float* __restrict__ tv2z,
...
int main() {
    printf("══════════════════════════════════════════════════════════════════════════\n");
    printf("  V25 — Occupancy A/B Test: 45reg vs maxreg=32 Primary\n");
    ...
    for (int si = 0; si < nScenes; si++) {
        ...
        // ──── PRIMARY HiOcc (maxreg=32, 100% occupancy) ────
        {
            unsigned int zero = 0; cudaMemcpyToSymbol(g_rayCounter, &zero, 4);
            tracePrimaryHiOcc<<<640, 256>>>(d_bvh4, n4, d_tv[0],d_tv[1],d_tv[2],d_tv[3],d_tv[4],d_tv[5],d_tv[6],d_tv[7],d_tv[8],
                d_ray[0],d_ray[1],d_ray[2],d_ray[3],d_ray[4],d_ray[5],d_ray[6],d_ray[7],d_ray[8], d_hits, NRAYS);
            cudaDeviceSynchronize();
            cudaEvent_t ht0, ht1; cudaEventCreate(&ht0); cudaEventCreate(&ht1);
            float hbest = 1e30f;
            for (int run = 0; run < 5; run++) {
                zero = 0; cudaMemcpyToSymbol(g_rayCounter, &zero, 4);
                cudaEventRecord(ht0);
                tracePrimaryHiOcc<<<640, 256>>>(d_bvh4, n4, d_tv[0],d_tv[1],d_tv[2],d_tv[3],d_tv[4],d_tv[5],d_tv[6],d_tv[7],d_tv[8],
                    d_ray[0],d_ray[1],d_ray[2],d_ray[3],d_ray[4],d_ray[5],d_ray[6],d_ray[7],d_ray[8], d_hits, NRAYS);
                cudaEventRecord(ht1); cudaEventSynchronize(ht1);
                float ms; cudaEventElapsedTime(&ms, ht0, ht1);
                if (ms < hbest) hbest = ms;
            }
            cudaMemcpy(h_hits, d_hits, NRAYS * sizeof(Hit), cudaMemcpyDeviceToHost);
            int hc2 = 0; for (int i = 0; i < NRAYS; i++) if (h_hits[i].tri >= 0) hc2++;
            printf("  PRIMARY (HiOcc):    %7.0f MR/s  hit:%5.1f%%\n", (float)NRAYS / hbest / 1000.f, 100.f * hc2 / NRAYS);
            cudaEventDestroy(ht0); cudaEventDestroy(ht1);
        }
